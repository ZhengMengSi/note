	...
	var FilterPanelHandler = function() {	
			...
			if (loCharFilters) {
				...
				for (var i = 0; i < tTokens.length; i++) {
					...
					if (loRange) {
				        ...
				        var lIsRange = true;
						if (!loRange.exclude && (!loRange.operation || loRange.operation === sap.zen.ValueHelpRangeOperation.EQ)) {
							lIsRange = false;
						}
		...
		this.createRangeFromToken = function(oModel, charName, oChar, oToken) {
			...
			if (oToken.data("range")) {			
				...
				lIsRange = true;
			} else if (oToken.data("input")) {			
				var loTokenRange = oToken.data("input");
				...
				lIsRange = lOperation !== sap.zen.ValueHelpRangeOperation.EQ || lExclude;
			} else {
				if (lLongKey) {
					lKeyFrom = lLongKey;
				}
