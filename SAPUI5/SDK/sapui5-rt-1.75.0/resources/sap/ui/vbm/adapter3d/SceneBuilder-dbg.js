...
function(jQuery, BaseObject, Utilities, THREE, ColladaLoader, DecalGeometry, html2canvas) {
	...
	SceneBuilder.prototype._setupView = function(position, zoom, pitch, yaw, home, flyTo) {
		...
		var state = {
			zoom: 1.0,
			target: new Vector3(-camTarget.x, -camTarget.z, camTarget.y), // convert from left handed (DirectX) to right handed (OpenGL)
			position: new Vector3(-pos.x, -pos.z, pos.y) // convert from left handed (DirectX) to right handed (OpenGL)
		};
	 ...
	 * @private
	 */
	SceneBuilder.prototype._getPolygonGeometry = function(posarray, ON) {
