...
sap.ui.define([
	...
	ActionUtil.prototype.call = function (sFunctionImportPath, sFunctionImportLabel, bIsDraftEnabled) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (that._isActionCritical()) {
				...
				mActionParams = that._prepareParameters(aContexts);
				if (aContexts.length > 1 && mActionParams && mActionParams.additionalParameters.length) {
					var sMsgBoxText = sap.ui.getCore().getLibraryResourceBundle("sap.ui.generic.app").getText("MULTI_SELECTION");
					MessageBox.error(sMsgBoxText, {});
					return;
				}
				mActionParams = mActionParams || {};
				mActionParams.isDraftEnabled = bIsDraftEnabled;
	...
	ActionUtil.prototype._prepareParameters = function (aContexts) {
		...
		if (Array.isArray(aContexts) && aContexts.length) {
			oSingleContext = aContexts[0]; // pick first item
		} else {
			return undefined;
		}
						...
						press: function (oEvent) {
							if (mParameterForm.hasNoClientErrors()) {
								if (mParameterForm.getEmptyMandatoryFields().length == 0) {
									...
									mActionParams.additionalParameters.forEach(function(param) {
										// Set default value(false) for boolean type if value is 'undefined'
										if (param.type == "Edm.Boolean" && _dataObject[param.name] == undefined) {
											oActionContext.oModel.setProperty(param.name, false, oActionContext);
										}
									});
