...
sap.ui.define([
	 ...
	 * @param {sap.ui.vk.Annotation} annotation Annotation you wish to add
	 *
	 * @returns {sap.ui.vk.Scene} <code>this</code> to allow method chaining.
	 *
	 * @public
	 ...
	 *
	 * @returns {array} Array of all annotations in Scene
	 *
	 * @public
	 */
	 ...
	 * @param {sap.ui.vk.Annotation} annotation Annotation you wish to remove
	 *
	 * @returns {sap.ui.vk.Scene} <code>this</code> to allow method chaining.
	 *
	 * @public
	 */
	Scene.prototype.removeAnnotation = function(annotation) {
		var index = this._indexOfAnnotation(annotation);
		if (index >= 0) {
	...
	Scene.prototype._indexOfAnnotation = function(annotation) {
		if (!this._annotations) {
			return -1;
		}

		return this._annotations.find(function(item) {
			return item == annotation;
		});
	};

	Scene.prototype.updateSceneAnnotations = function(nodeRefs, titles, styles) {
		...
		nodeRefs.forEach(function(nodeRef, i) {
			...
			var matchNode = this.getAnnotations().find(function(atn) {
				return atn.getNodeRef().uuid === node.uuid;
			});
			if (matchNode) {
				if (titles && titles[i] && titles[i] !== matchNode.getTitle()) {
					matchNode.setProperty("title", titles[i]);
				}
				if (styles && styles[i] && styles[i] !== matchNode.getStyle()) {
					matchNode.setProperty("style", styles[i]);
				}
				matchNode.setVisible(true);
			} else {
				var title = titles && titles[i] ? titles[i] : nodeRef.name;
				var style = styles && styles[i] ? styles[i] : sap.ui.vk.AnnotationStyle.Default;
				var atn = new Annotation({ nodeRef: node, title: title, style: style });
				newAnnotations.push(atn);
			}
		}.bind(this));
		newAnnotations.forEach(function(a) {
