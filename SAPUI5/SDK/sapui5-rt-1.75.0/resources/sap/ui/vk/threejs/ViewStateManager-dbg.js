...
sap.ui.define([
	...
	*/
	...
	ViewStateManager.prototype._getCurrentView = function() {
			...
			nodeInfo.forEach(function(node) {
				...
				if (!property || property !== "opacity") {
					...
					if (newPosition) {
						...
						this.setTransformation(transforms.nodeRefs, transforms.positions, true);
					}
				} else {
	...
	*/
	 ...
	 * @param {boolean} initial Indicates that transformations are initial.
	 * @returns {sap.ui.vk.ViewStateManager} <code>this</code> to allow method chaining.
	 * @private
	 */
	ViewStateManager.prototype.setTransformation = function(nodeRefs, transformations, initial) {
		...
		if (!transformations) {
			...
			nodeRefs.forEach(function(nodeRef) {
				if (nodeRef.userData.position && nodeRef.userData.quaternion && nodeRef.userData.scale) {
					...
					delete nodeRef.userData.invMatrix;
					delete nodeRef.userData.animMatrix;
				}
			...
			nodeRefs.forEach(function(nodeRef, idx) {
				...
				if (initial || !userData.animMatrix) {
					userData.invMatrix = (userData.invMatrix || new THREE.Matrix4()).getInverse(nodeRef.matrix);
					userData.animMatrix = (userData.animMatrix || new THREE.Matrix4()).identity();
				} else {
					userData.animMatrix.multiplyMatrices(userData.invMatrix, nodeRef.matrix);
				}
	...
	ViewStateManager.prototype._clearJointNodes = function() {
		if (this._jointCollection && this._jointCollection.length > 0) {
			this._jointCollection.forEach(function(joint) {
				var node = joint.node;
				if (node && node.userData) {
					delete node.userData.invMatrix;
					delete node.userData.animMatrix;
				}

				var parent = joint.parent;
				if (parent && parent.userData) {
					delete parent.userData.invMatrix;
					delete parent.userData.animMatrix;
				}

				if (joint.nodesToUpdate) {// update dependent intermediate nodes
					joint.nodesToUpdate.forEach(function(subnode) {
						if (subnode && subnode.userData){
							delete subnode.userData.invMatrix;
							delete subnode.userData.animMatrix;
						}
	...
	ViewStateManager.prototype._updateJointNodes = function() {
		if (this._jointCollection && this._jointCollection.length > 0) {
			this._jointCollection.forEach(function(joint) {
				...
				if (node.userData && node.userData.animMatrix === null) {
					return;
				}
				...
				if (node.userData.animMatrix) {// apply node animation to the joint node
					node.matrixWorld.multiply(node.userData.animMatrix);
				}
	...
	ViewStateManager.prototype._setForSkippingJoints = function(nodes) {
		if (this._jointCollection && this._jointCollection.length > 0) {
			var jointMap = new Map();
			this._jointCollection.forEach(function(joint) {
				jointMap.set(joint.node, joint);
