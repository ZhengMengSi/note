...
sap.ui.define([
	...
	ViewportRenderer.render = function(rm, control) {
		...
		// Render annotations
		if (control.getScene() && control.getScene().getAnnotations().length > 0){
			var oAnnotations = control.getScene().getAnnotations();
			rm.write("<div");
			rm.addClass("sapUiVizKitAnnotationContainer");
			rm.writeClasses();
			rm.write(">");
			for (i = 0, l = oAnnotations.length; i < l; i++) {
				rm.renderControl(oAnnotations[i]);
			}
			rm.write("</div>");
		}
