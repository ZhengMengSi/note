	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		(function() { // constructor coding encapsulated in order to reduce scope of helper variables
			...
			if (oQuickVariantSelectionEffective) {
				var enableAutoBindingMultiView = oQuickVariantSelectionEffective.enableAutoBinding;		//Check if value of enableAutoBinding is being set from manifest
				if (enableAutoBindingMultiView === null || enableAutoBindingMultiView === undefined) {
					oQuickVariantSelectionEffective.enableAutoBinding = true;							//set the default value of enableAutoBinding to true
				}
			}
			if (!oQuickVariantSelectionEffective) {
				return;
