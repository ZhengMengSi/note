...
`		@param {object} App data
		...
		*/
		...
		function fnAdaptToAppState(oAppData, oURLParameters, sNavType){
			...
			if (oState.oWorklistData.bWorkListEnabled) {
				...
				if (bDataAreShownInTable === null || bDataAreShownInTable === undefined) {
					bDataAreShownInTable = bIsCrossAppNavigation || oState.bLoadListAndFirstEntryOnStartup || bIsAutoBinding || oState.oMultipleViewsHandler.getEnableAutoBinding();
				}
				var bIsMandatoryFilterNotFilled = checkForMandatoryFilters();													// Check if manadatory filters are not filled
				if (bIsMandatoryFilterNotFilled && oState.oSmartFilterbar.isCurrentVariantStandard() && !oState.oSmartFilterbar.getSmartVariant().bExecuteOnSelectForStandardByUser && oState.oSmartFilterbar.getSmartVariant().bExecuteOnSelectForStandardViaXML ) {
					//set Apply Automatically and data shown to false if mandatory filters are not filled
		...
		*/
		function onSFBVariantInitialise() {
			bIsAutoBinding = oState.oSmartTable.getEnableAutoBinding() || oState.oMultipleViewsHandler.getEnableAutoBinding();
			if (bIsAutoBinding) {
