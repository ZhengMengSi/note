        ...
        Promise
            ...
            .then(function (aResult) {
                ...
                var oSite = {
                    ...
                    catalogs: {},
                    groups: {},
                    visualizations: cdmSiteUtils.getVisualizations(oVisualizationData),
    ...
    PagesCommonDataModelAdapter.prototype._addPageToSite = function (site, page, navigationData) {
        ...
        for (var j = 0; j < page.sections.length; j++) {
            ...
            oPageSection = oPage.payload.sections[oSection.id] = {
                ...
                payload: {
                    layout: {
                        vizOrder: oSection.visualizations.map(function (visualization) {
            ...
            var oViz;
            var oPageSectionPayload;
            for (var k = 0; k < oSection.visualizations.length; k++) {
                oViz = oSection.visualizations[k];
                oPageSectionPayload = oPageSection.payload;
                // Skip invalid visualizations
                if (!site.visualizations[oViz.vizId]) {
                    // Remove the invalid visualization from the vizOrder
                    var aVizOrder = oPageSectionPayload.layout.vizOrder;
                ...
                oPageSectionPayload.viz[oViz.id] = {
                    id: oViz.id,
                    vizId: oViz.vizId
                };

                var oVisualizationTarget = site.visualizations[oViz.vizId].vizConfig["sap.flp"].target;
                if (oVisualizationTarget.type !== "URL") {
                    oVisualizationTarget.appId = oViz.inboundPermanentKey;
                    oVisualizationTarget.inboundId = oNavigationDataHashMap[oViz.inboundPermanentKey] && oNavigationDataHashMap[oViz.inboundPermanentKey].id;
                }
            }
        }
    };
