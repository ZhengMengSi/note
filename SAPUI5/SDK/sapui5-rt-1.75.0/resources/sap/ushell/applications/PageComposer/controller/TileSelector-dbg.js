...
sap.ui.define([
    ...
    return function () {
        ...
        /**
         * Consumes catalog data and builds the catalog tree, replacing the TileSelectorList (sap.m.Tree) model with it.
         * Usually called with the returned data from PagePersistence's "getCatalogs" method.
         *
         * @param {object[]} aCatalogs The catalogs.
         * @see sap.ushell.applications.PageComposer.util.PagePersistence#getCatalogs
         * @private
         */
        this.initTiles = function (aCatalogs) {
            if (typeof aCatalogs === "undefined") {
                throw new Error("sap.ushell.applications.PageComposer.controller.TileSelector#initTiles: undefined argument");
            }
            var catalogs = fnClone(aCatalogs);
            catalogs.forEach(function (catalog) {
                catalog.visualizations = catalog.visualizations.results.sort(function (a, b) {
                    // sorts tiles by title in ascending lexicographical order
                    if (a.title > b.title) { return 1; }
                    if (a.title < b.title) { return -1; }
                    return 0;
                });
                catalog.visualizations.forEach(function (visualization) {
                    delete visualization.__metadata; // delete "__metadata" to avoid the TileSelector Tree from rendering it
                });
                delete catalog.roles; // delete "roles" to avoid the TileSelector Tree from rendering it
                delete catalog.__metadata; // delete "__metadata" to avoid the TileSelector Tree from rendering it
            });
            this._setCatalogTree(catalogs);
        }.bind(this);
        ...
        this._onSelectionChange = function (oEvent) {
            ...
            if (oEvent) {
                oEvent.getParameters().listItems.forEach(function (item) {
                    if (!item.getBindingContext().getProperty("vizId")) { // catalog (root item)
                        item.setSelected(false); // currently, catalogs should not be selectable
                        _toggleCollapseTreeItem(item); // instead, allow toggling collapse with spacebar
                    }
                });
            }
         ...
         * @private
         */
        this._setCatalogTree = function (aCatalogTree) {
            oAddSelectedTilesButton.setEnabled(false);
        ...
        /**
         * Toggles the lexicographical sort order of the Tree between "ascending" and "descending".
        ...
        function sortCatalogsToggle () {
            ...
            var oItems = oTree.getBinding("items"),
                oSorterCatalog = new Sorter("title", bSortAscending);
