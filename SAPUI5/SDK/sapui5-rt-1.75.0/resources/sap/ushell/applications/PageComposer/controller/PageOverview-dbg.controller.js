    ...
    return BaseController.extend("sap.ushell.applications.PageComposer.controller.Main", {
        ...
        onErrorMessageClicked: function (oEvent) {
            var oSelectedObject = oEvent.getSource().getBindingContext().getObject(),
                oFilteredErrorMsgByPageId = this.onfilterErrorMessages(oSelectedObject.id);
            if (oFilteredErrorMsgByPageId.length) {
                this.showMessageBoxWarning(
                    this.getResourceBundle().getText("Message.PageTemplateId", [oSelectedObject.id])
                    + ": " + oFilteredErrorMsgByPageId[0].message,
                    false
                );
            }
        },
            ...
            sap.ui.require([
                ...
                Fragment.load({
                    ...
                    if (!this.getModel().getData().transportSupported) {
                        oFragment.removeFilterItem("PackageFilter");
                        oFragment.removeFilterItem("WorkbenchRequestFilter");
                        oFragment.removeSortItem("PackageSort");
                        oFragment.removeSortItem("WorkbenchRequestSort");
                        oFragment.removeGroupItem("PackageGroup");
                        oFragment.removeGroupItem("WorkbenchRequestGroup");
                    }
