    ...
    function _setRoleContext (aRoles, aSelectedRoles) {
        if (_oDialog) {
            ...
            var bAllSelected = !aSelectedRoles.length || aSelectedRoles.length === aRoles.length,
                sActiveRoleContextInfo = bAllSelected ? _sAllText : aSelectedRoles.length || "";

            oModel.setData({
                allRoles: aRoles,
                selectedRoles: aSelectedRoles,
                roleContextInfo: sActiveRoleContextInfo,
                showRoleContextInfo: !bAllSelected
            });
            if (aRoles.length) {
        ...
        open: function (oParentView) {
            ...
            this.load(oParentView.getId()).then(function (oDialog) {
                ...
                var oParentRolesModel = oParentView.getModel("roles"),
                    oData = oParentRolesModel && oParentRolesModel.getData() || {},
                    sRoleContextInfo = oData.activeRoleContextInfo || _sAllText,
                    aSelectedRoles = oData.activeRoleContext || [],
                    oRolesModel = new JSONModel({
                        allRoles: [],
                        selectedRoles: aSelectedRoles,
                        roleContextInfo: sRoleContextInfo,
                        showRoleContextInfo: false
                    });
        ...
        onOpenContextSelector: function () {
            sap.ui.require([
                ...
                if (!this.oContextSelectorController) {
                    this.oContextSelectorController = new ContextSelector(_oParentView, _oParentView.getController().getResourceBundle());
                }
                var oModel = _oDialog.getModel("roleContext"),
