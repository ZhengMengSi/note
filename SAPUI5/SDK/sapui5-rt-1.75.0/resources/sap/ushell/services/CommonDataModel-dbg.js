...
sap.ui.define([
    ...
    "sap/ushell/Config"
], function (PersonalizationProcessor, ObjectPath, jQuery, isPlainObject, SiteConverter, isEmptyObject, deepClone, Version, Config) {
    ...
    CommonDataModel.prototype._loadAndApplyPersonalization = function (oSite) {
        ...
        if (oCDMSiteVersion.compareTo("3.1.0") < 0) {
            this._oAdapter.getPersonalization(oCDMSiteVersion)
                .then(function (oPers) {
                    if (oPers) {
                        ...
                        this._mPersonalizationDeltas = { version: "3.0.0" };
                    }
            ...
            this._oSiteDeferred.resolve(oSite);
        }
    };
    /**
    ...
    CommonDataModel.prototype.getPage = function (sPageId) {
        return new Promise(function (resolve, reject) {
            if (!this._oAdapter.getPage) {
                reject("The configured CDM adapter doesn't support the CDM 3.1 pages: Function 'getPage' not implemented.");
                return;
            }
    ...
    /**
     * Returns the Common Data Model site of a Page
     * @param {array} aPageId the Id of the page to return
     *
     * @returns {Promise<object>}
     *    resolves with the Common Data Model site
     * @since 1.75.0
    ...
    CommonDataModel.prototype.getPages = function (aPageId) {
        if (!this._oAdapter.getPages) {
            return Promise.reject("The configured CDM adapter doesn't support the CDM 3.1 pages: Function 'getPages' not implemented.");
        }

        var aPagesPromises = [];

        return this._oAdapter.getPages(aPageId).then(function (oPages) {
            function _getPersonalisedPage (sCurrentPageId, oCurrentPage) {
                return new Promise(function (resolve, reject) {
                    this._oAdapter.getPersonalization(this._oOriginalSite._version)
                        .done(function (personalisation) {
                            var oPage = oCurrentPage,
                                oPersonalization = personalisation,
                                oConvertedPage = this._oSiteConverter.convertTo("3.0.0", oPage);

                            this._oOriginalSite.pages[oPage.identification.id] = deepClone(oPage);
                            this._mPersonalizationDeltas = deepClone(oPersonalization);
                            var oPagePersonalization = oPersonalization[sCurrentPageId] || {};

                            this._triggerMixinPersonalisationInSiteAndStore.call(this, oConvertedPage, oPagePersonalization, sCurrentPageId)
                                .then(resolve)
                                .catch(function () {
                                    reject("Personalization Processor: Cannot mixin the personalization.");
                                });
            ...
            var sCurrentPageId, oCurrentPage;
            for (var i = 0; i < aPageId.length; i++) {
                sCurrentPageId = aPageId[i];
                oCurrentPage = oPages[sCurrentPageId];
                if (Config.last("/core/shell/enablePersonalization")) {
                    aPagesPromises.push(_getPersonalisedPage.call(this, sCurrentPageId, oCurrentPage));
                } else {
                    aPagesPromises.push(Promise.resolve(oCurrentPage));
                }
            }
            return Promise.all(aPagesPromises);
        }.bind(this));
    };
        ...
        this._oPersonalizationProcessor.extractPersonalization(oPersonalizedPage, oOriginalPage)
            .done(function (oExtractedPersonalization) {
                if (!isEmptyObject(oExtractedPersonalization)) {
                    ...
                    if (this._mPersonalizationDeltas.version === undefined) {
