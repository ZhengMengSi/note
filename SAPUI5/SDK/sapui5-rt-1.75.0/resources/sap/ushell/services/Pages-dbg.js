...
sap.ui.define([
        ...
        return Promise.all([
            ...
            .then(function (aServices) {
                ...
                return Promise.all([
                    ...
                    oVisualizationLoadingService.loadVisualizationData()
                ])
                    .then(function (aResults) {
                        ...
                        this._oPagesModel._setProperty(sNewPagePath, this._getModelForPage(oPage, oVisualizations, oApplications));
                        return sNewPagePath;
                    }.bind(this))
                    .catch(function (oError) {
                        Log.error("Pages: Failed to gather site data.", oError, this.COMPONENT_NAME);
                        return Promise.reject(oError);
                    }.bind(this));
            }.bind(this))
            .catch(function (oError) {
                Log.error("Pages: Couldn't resolve CDM Service or Visualization Loading Service.", oError, this.COMPONENT_NAME);
                return Promise.reject(oError);
            }.bind(this));
    };
     ...
     */
    Pages.prototype.findVisualization = function (sVizId, sPageId) {
     ...
     *
     * @returns {Promise<void>} Returns a promise which resolves to an empty value
     * @since 1.75.0
     *
     * @private
     */
    Pages.prototype.moveVisualization = function (iPageIndex, iSourceSectionIndex, iSourceVisualizationIndex, iTargetSectionIndex, iTargetVisualizationIndex) {
        ...
        return this._oCdmServicePromise.then(function (oCdmService) {
            oCdmService.getPage(sPageId).then(function (page) {
                var oSourceSection = page.payload.sections[sSourceSectionId];
                var aSourceVizOrder = oSourceSection.payload.layout.vizOrder;
                var oSourceViz = oSourceSection.payload.viz;

                var oTargetSection = page.payload.sections[sTargetSectionId];
                var aTargetVizOrder = oTargetSection.payload.layout.vizOrder;
                var oTargetViz = oTargetSection.payload.viz;
                ...
                // If the default section becomes empty, delete it
                if (oSourceSection.payload.default && !Object.keys(oSourceViz).length) {
                    delete page.payload.sections[sSourceSectionId]; // delete section from sections
                    page.payload.layout.sectionOrder.splice(iSourceSectionIndex, 1); // delete index from sectionOrder
                }

                this.savePersonalization(sPageId);
            }.bind(this)).catch(function () {
                Log.error("Personalization cannot be saved: CDM Service cannot retrieve the required page");
            }).catch(function () {
                Log.error("Personalization cannot be saved: CDM Service cannot be retrieved");
            });
        }.bind(this));
    };
     ...
     *
     * @returns {Promise} Promise which resolves after the CDM page was adapted.
     *
     * @since 1.75.0
     * @private
     */
    Pages.prototype.deleteVisualization = function (iPageIndex, iSourceSectionIndex, iSourceVisualizationIndex) {
        ...
        return this._oCdmServicePromise
            ...
            .then(function (oPage) {
                var aSectionVizOrder = oPage.payload.sections[oSectionModel.id].payload.layout.vizOrder;
                var aVizRefs = oPage.payload.sections[oSectionModel.id].payload.viz;
                ...
                this.savePersonalization(oPage.identification.id);
            }.bind(this));
    };
     ...
     * @param {object} mVisualizations A map of all visualization.
     * @param {object} mApplications A map of all applications.
     * @param {object} [oAddtionalVizData] Addtional visualization data that sould overwrite the standard data.
     * @param {string} sPageId Id of the Page
     *
     ...
     * @private
     */
    Pages.prototype._getVisualizationData = function (sVizId, mVisualizations, mApplications, oAddtionalVizData, sPageId) {
        var oVisualizationReference = oAddtionalVizData || {};

        var sAppId = readVisualizations.getAppId(mVisualizations[sVizId]),
            aCdmParts = [
                oVisualizationReference,
                readVisualizations.getConfig(mVisualizations[sVizId]),
                readHome.getInbound(mApplications[sAppId]),
                mApplications[sAppId]
            ];

        return {
            id: oVisualizationReference.id || this._generateId(sPageId),
            vizId: sVizId || "",
            vizType: readVisualizations.getTypeId(mVisualizations[sVizId]) || "",
            ...
            target: readVisualizations.getTarget(mVisualizations[sVizId]) || {}
        };
    };
     ...
     * @protected
     */
    Pages.prototype.addVisualization = function (sVizId, sPageId, sSectionId) {
        return this._oCdmServicePromise.then(function (oCdmService) {
            return Promise.all([
                this.loadPage(sPageId),
                oCdmService.getVisualizations(),
                ...
                var sPagePath = aResult[0],
                    ...
                    oVisualizationData = this._getVisualizationData(sVizId, aResult[1], aResult[2], {}, sPageId);
                ...
                if (iSectionIndex !== undefined || iDefaultSectionIndex !== undefined) {
                    ...
                    return oCdmService.getPage(sPageId).then(function (oPage) {
                        var oSection = oPage.payload.sections[sSectionId || oPage.payload.layout.sectionOrder[0]];

                        oSection.payload.layout.vizOrder.push(oVisualizationData.id);
                        oSection.payload.viz[oVisualizationData.id] = {
     ...
     * @returns {Promise} Returns a promise which resolves to an empty value
     *
     * @since 1.75.0
     *
     * @private
    ...
    Pages.prototype.moveSection = function (iPageIndex, iSourceSectionIndex, iTargetSectionIndex) {
        ...
        return this._oCdmServicePromise.then(function (oCdmService) {
            oCdmService.getPage(sPageId).then(function (page) {
                var aSectionOrder = page.payload.layout.sectionOrder;
     ...
     * @returns {Promise<void>} A promise that is resolved once the CDM page has been fetched.
     *
     * @since 1.75.0
     *
     * @private
    ...
    Pages.prototype.addSection = function (iPageIndex, iSectionIndex, oSectionProperties) {
        ...
        return this._oCdmServicePromise.then(function (oCommonDataModelService) {
            return oCommonDataModelService.getPage(sPageId).then(function (oPage) {
                var oSectionPayload = {
                ...
                if (oNewSection.visualizations) {
                    ...
                    for (; i < oNewSection.visualizations.length; i++) {
                        ...
                        oSectionPayload.layout.vizOrder.push(oVizData.id);
                        if (oVizData.isBookmark) {
                            oSectionPayload.viz[oVizData.id] = oVizData;
                        } else {
                            oSectionPayload.viz[oVizData.id] = {
                                id: oVizData.id,
     ...
     * @returns {Promise<object>} Returns a promise which resolves to the adapted page
     *
     * @since 1.75.0
     *
     * @private
        ...
        return this._oCdmServicePromise
            .then(function (oCdmService) {
                return oCdmService.getPage(sPageId)
                    ...
                    .catch(function (oError) {
                        Log.error("Pages: Failed to get Page from CDM Service.", oError, this.COMPONENT_NAME);
                        this.cancelPersonalization();
                        return Promise.reject(oError);
                    }.bind(this));
            }.bind(this))
            .catch(function (oError) {
                Log.error("Pages: Failed to get CDM Service.", oError, this.COMPONENT_NAME);
                this.cancelPersonalization();
                return Promise.reject(oError);
            }.bind(this));
    };
     ...
     * @returns {Promise<any>} A Promise that resolves to the adapted Page or an empty value.
     *
     * @since 1.75.0
     *
     * @private
    ...
    Pages.prototype.setSectionVisibility = function (iPageIndex, iSectionIndex, bVisibility) {
        ...
        return this._oCdmServicePromise
            .then(function (oCdmService) {
                return oCdmService.getPage(sPageId)
                    .then(function (oPage) {
                        oPage.payload.sections[sSectionId].payload.visible = bVisibility;
                        this.savePersonalization(sPageId);
                        return Promise.resolve(oPage);
                    }.bind(this))
                    .catch(function (oError) {
     ...
     *
     * @returns {Promise<object>} The updated CDM page.
     *
     * @since 1.75.0
     * @private
     */
    Pages.prototype.renameSection = function (iPageIndex, iSectionIndex, sNewTitle) {
        ...
        return this._oCdmServicePromise
            ...
            .then(function (oPage) {
                oPage.payload.sections[oSectionModel.id].identification.title = sNewTitle;
                this.savePersonalization(oPage.identification.id);
            }.bind(this));
    };
     ...
     * @param {int} iSectionIndex The index of the section.
     *
     * @returns {Promise<void>} A Promise that resolves to an empty value.
     *
     * @since 1.75.0
     *
     * @private
            ...
            .then(function (oCdmService) {
                return Promise.all([
                    ...
                    .then(function (aService) {
                        var oVisualizations = aService[0];
                        var oApplications = aService[1];
                        var oCdmPage = aService[2];
                        var oOriginalCdmPage = oCdmService.getOriginalPage(sPageId);

                        var oOriginalPageModel = this._getModelForPage(oOriginalCdmPage, oVisualizations, oApplications);
                        var oOriginalSectionModel = deepClone(oOriginalPageModel.sections.find(function (section) { return section.id === sSectionId; }));

                        var aOriginalVizIds = oOriginalSectionModel.visualizations.map(function (oVisualization) {
                            return oVisualization.id;
                        });

                        // the following loop ensures unique ids for viz refrences within a page according to adr-1011
                            ...
                            if (oOriginalSectionModel.id !== oCurrentSectionModel.id) {
                                oCurrentSectionModel.visualizations.forEach(function (oVisualization) {
                                    if (aOriginalVizIds.indexOf(oVisualization.id) !== -1) {
                                        ...
                                        var oVizRef = deepClone(oCdmPage.payload.sections[oCurrentSectionModel.id].payload.viz[oVisualization.id]);
                                        delete oCdmPage.payload.sections[oCurrentSectionModel.id].payload.viz[oVisualization.id];
                                        var iVizOrderIndex = oCdmPage.payload.sections[oCurrentSectionModel.id].payload.layout.vizOrder.indexOf(oVizRef.id);

                                        oVizRef.id = sNewId;
                                        oCdmPage.payload.sections[oCurrentSectionModel.id].payload.viz[sNewId] = oVizRef;
                                        oCdmPage.payload.sections[oCurrentSectionModel.id].payload.layout.vizOrder[iVizOrderIndex] = sNewId;
                    ...
                    .catch(function (oError) {
                        Log.error("Pages: Failed to gather data from CDM Service.", oError, this.COMPONENT_NAME);
                        return Promise.reject(oError);
                    }.bind(this));
            }.bind(this))
            .catch(function (oError) {
                Log.error("Pages: Couldn't resolve CDM Service.", oError, this.COMPONENT_NAME);
                this.cancelPersonalization();
                return Promise.reject(oError);
            }.bind(this));
    };
            ...
            .then(function (oCdmService) {
                return Promise.all([
                    ...
                    .then(function (aService) {
                        ...
                        this.savePersonalization(oCdmPage.identification.id);
                    }.bind(this))
                    .catch(function (oError) {
                        Log.error("Pages: Failed to gather data from CDM Service.", oError, this.COMPONENT_NAME);
                        return Promise.reject(oError);
    ...
    /**
     * Sets the dirty state on true and copies the model data
     *
     * @since 1.74.0
     *
     * @private
     */
    Pages.prototype._setDirtyState = function () {
        if (!this._bDirtyState) {
            this._bDirtyState = true;
            this._oCopiedModelData = deepClone(this._oPagesModel.getProperty("/"));
        }
    };
     ...
     *
     * @returns {Promise<void>} A promise which resolves after saving
     *
     * @since 1.74.0
     * @private
     */
    Pages.prototype.savePersonalization = function (sPageId) {
        return this._oCdmServicePromise.then(function (oCDMService) {
            return oCDMService.save(sPageId).then(function () {
                Log.info("Save CDM Model");
    ...
    /**
     * Cancels the personalization by setting the copied modelData to the actual model.
     *
     * @since 1.74.0
     *
     * @private
     */
    Pages.prototype.cancelPersonalization = function () {
        if (this._bDirtyState) {
     ...
     *
     * @param {object} page One CDM 3.1 page
     * @param {object[]} visualizations All the visualizations of the CDM 3.1 site
     * @param {object[]} applications All the applications of the CDM 3.1 site
     *
     * @returns {object} An object which represents the page inside the Pages Service JSON Model
     *
     * @since 1.75.0
     * @private
     */
    Pages.prototype._getModelForPage = function (page, visualizations, applications) {
        var oPage = {
        ...
        page.payload.layout.sectionOrder.forEach(function (sSectionId) {
            var oCDMPageSection = page.payload.sections[sSectionId];
            var oSection = {
                id: oCDMPageSection.identification.id || "",
                title: oCDMPageSection.identification.title || "",
                visualizations: [],
                visible: oCDMPageSection.payload.visible !== undefined ? oCDMPageSection.payload.visible : true,
                locked: oCDMPageSection.payload.locked !== undefined ? oCDMPageSection.payload.locked : false,
                preset: oCDMPageSection.payload.preset !== undefined ? oCDMPageSection.payload.preset : true,
                default: oCDMPageSection.payload.default !== undefined ? oCDMPageSection.payload.default : false
            };

            oCDMPageSection.payload.layout.vizOrder.forEach(function (id) {
                var oVisualizationReference = oCDMPageSection.payload.viz[id],
                    sVizId = oVisualizationReference.vizId,
                    oVisualizationData = oVisualizationReference.isBookmark ? oVisualizationReference : this._getVisualizationData(sVizId, visualizations, applications, oVisualizationReference, page.identification.id);

                oSection.visualizations.push(oVisualizationData);
            }.bind(this));
     ...
     *
     * @returns {Promise<void>} A promise which resolves to an empty value.
     *
     * @since 1.75.0
     *
     * @private
     */
    Pages.prototype.addBookmarkToPage = function (bookmark, pageId) {
        if (!pageId) {
            return Promise.reject("Pages Service - Adding bookmark tile failed: No page id is provided.");
        }
        ...
        return this.loadPage(pageId)
            .then(function (pagePath) {
                ...
                var oVisualizationModelData = bookmark;
                oVisualizationModelData.isBookmark = true;
                oVisualizationModelData.id = this._generateId(pageId);
                ...
                if (!oDefaultSection) {
                    return this.addSection(iPageIndex, 0, {
                        title: resources.i18n.getText("DefaultSection.Title"),
                        default: true,
                        visualizations: [oVisualizationModelData]
                    });
                }

                // Add visualization to existing default section, update model & site, save personalization
                oDefaultSection.visualizations.push(oVisualizationModelData);
                this._oPagesModel.refresh();
                return this._oCdmServicePromise
                    .then(function (oCdmService) {
                        oCdmService.getPage(pageId)
                            .then(function (page) {
                                var oSection = page.payload.sections[oDefaultSection.id];

                                oSection.payload.layout.vizOrder.push(oVisualizationModelData.id);
                                oSection.payload.viz[oVisualizationModelData.id] = oVisualizationModelData;
                                // Save
                                this.savePersonalization(pageId);
                            }.bind(this));
                    }.bind(this));
            }.bind(this))
            .catch(function (error) {
                Log.error("Pages Service - Insert New Bookmark Tile Failed");
