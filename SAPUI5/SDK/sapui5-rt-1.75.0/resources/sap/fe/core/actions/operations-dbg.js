...
sap.ui.define(
	...
	function(MessageBox, Dialog, JSONModel, XMLTemplateProcessor, XMLPreprocessor, Fragment, messageHandling, BusyLocker, TableRuntime) {
		"use strict";
		...
		function confirmCriticalAction(sActionName, sActionLabel, mParameters) {
			return new Promise(function(resolve, reject) {
		...
		function showActionParameterDialog(sActionName, sActionLabel, mParameters, aActionParameters, oActionContext, oParentControl) {
					...
					return setUserDefaults(mParameters.ownerComponent, aActionParameters, oParameterModel).then(function() {
						...
						Fragment.load({ definition: oFragment }).then(function(oDialogContent) {
												...
												for (i = 0; i < aActionParameters.length; i++) {
													...
													if (!value && mRequiredProperties[sParameterBinding]) {
														...
														oDialog.fireValidationError({
															...
															newValue: value,
															message: oResourceBundle.getText("SAPFE_ACTION_PARAMETER_REQUIRED")
														});
							...
							if (aContexts.length === 1) {
								...
								oOperationBinding.setParameter("_it", aContexts[0].getObject());
							}
							...
							var getParameterDefaultValue = function(sParamName) {
								...
								if (
									...
									oParameterDefaultValue.$Path.startsWith(sItParameter + "/")
								) {
						...
						fnExecuteAction = function(oAction, index, oSideEffect) {
							...
							if (oSideEffect && oSideEffect.pathExpressions) {
								oSideEffect.context.requestSideEffects(oSideEffect.pathExpressions, sGroupId).then(function() {
									TableRuntime.setActionEnablement(
