...
sap.ui.define(
	...
	function(
		...
		var sFragmentName = "sap.fe.core.controls.field.DraftPopOverAdminData",
			...
			mUIStateModels = {}, // Map AppID --> UIStateModel
		...
		var Extension = ControllerExtension.extend("sap.fe.core.controllerextensions.EditFlow", {
							...
							.then(function(sCreationMode) {
								...
								if (sCreationMode !== "Deferred") {
									...
									oCreation = oValidationCheck.then(function() {
										...
										return transactionHelper.createDocument(oListBinding, mParameters);
									});
								}
			...
			saveDocument: function(oContext) {
				...
				return (
					this.syncTask()
						...
						.then(transactionHelper.saveDocument.bind(transactionHelper, oContext))
			...
			cancelDocument: function(oContext, mParameters) {
				...
				this.syncTask()
					.then(transactionHelper.cancelDocument.bind(transactionHelper, oContext, mParameters))
					.then(function(oActiveDocumentContext) {
						transactionHelper.getProgrammingModel(oContext).then(function(sProgrammingModel) {
							var bNoHashChange;
			...
			deleteMultipleDocuments: function(oContext, mParameters) {
				...
				this._deleteDocumentTransaction(oContext, mParameters).then(function() {
					...
					var oRouterProxy = CommonUtils.getAppComponent(that.base.getView()).getRouterProxy();
					var bImpacted = false;
			...
			_deleteDocumentTransaction: function(oContext, mParameters) {
				...
				return this.syncTask()
					.then(transactionHelper.deleteDocument.bind(transactionHelper, oContext, mParameters, oLocalUIModel))
					.then(function() {
						var oLocalUIModel = that.base.getView().getModel("localUI");
						oLocalUIModel.setProperty("/sessionOn", false);
					});
				...
				return this._submitOpenChanges(oContext)
					...
					.catch(function(err) {
						...
						aCustomMessages.push({
							text: transactionHelper.getText("SAPFE_APPLY_ERROR"),
							type: "Error"
						});
			...
			formatDraftOwnerText: function(
				...
				if (sFlag) {
					sDraftOwnerDescription += sDraftInProcessByUser
						? transactionHelper.getText("DRAFTINFO_GENERIC_LOCKED_OBJECT_POPOVER_TEXT") + " "
						: transactionHelper.getText("DRAFTINFO_LAST_CHANGE_USER_TEXT") + " ";
				}
				sDraftOwnerDescription += sUserDescription
					? transactionHelper.getText("DRAFTINFO_OWNER", [sUserDescription])
					: transactionHelper.getText("DRAFTINFO_ANOTHER_USER");
			...
			handlePatchEvents: function(oBinding) {
				// TODO: the draft indicator shall be handled in the transaction helper
			...
			getUIStateModel: function() {
				/*
					some states are handled by the transaction helper. in the usage of the edit flow controller
					...
					instances of the edit flow and transaction helper for this app
				...
				if (!this.editFlowStateModel) {
					...
					if (!mUIStateModels[sAppComponentId]) {
						mUIStateModels[sAppComponentId] = transactionHelper.getUIStateModel();
					} else {
						// pass the UI State model to the transaction helper
						transactionHelper.setUIStateModel(mUIStateModels[sAppComponentId]);
					}
		...
		/**
		 * Deletes the UIStateModel and the TransactionHelper for a given application
		 */
		Extension.onExitApplication = function(sAppId) {
			mUIStateModels[sAppId] = undefined;
			mTransactionHelpers[sAppId] = undefined;
		};
