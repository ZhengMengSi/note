	...
	function(
		...
		var Extension = ControllerExtension.extend("sap.fe.core.controllerextensions.Routing", {
			...
			navigateToContext: function(oContext, mParameters) {
				...
				var oRouterProxy = this._getOwnerComponent().getRouterProxy();
				...
				if (mParameters.asyncContext || mParameters.deferredContext) {
					...
					if (sTempPath !== "") {
						if (sHash === sTempPath) {
							this._bindTargetPage(oRootControl.getCurrentPage(), null, null, true);
							return Promise.resolve();
				...
				if (oFcl.isFclEnabled()) {
					...
					} else if (oRouteDetail && oRouteDetail.layout) {
				...
				if (mParameters.noHashChange) {
					...
					if (oFcl.isFclEnabled()) {
						...
						// oRootControl is a FlexibleColumnLayout
						if (
							sPath ===
							...
							var oTargetPage;
							switch (oViewData.FCLLevel) {
								case 0:
									oTargetPage = oRootControl.getCurrentBeginColumnPage();
									break;

								case 1:
									oTargetPage = oRootControl.getCurrentMidColumnPage();
									break;

								default:
									oTargetPage = oRootControl.getCurrentEndColumnPage();
									break;
							}
						...
						// In the fullscreen case, oRootControl is a NavContainer
						this._bindTargetPage(oRootControl.getCurrentPage(), null, null, true);
						return Promise.resolve();
					}
				}
			...
			navigateToMessagePage: function(sErrorMessage, mParameters) {
				if (
					...
					var oAppComponent = mParameters.navContainer.getParent();
			...
			initializeRouting: function(oAppComponent) {
				...
				return new Promise(function(resolve, reject) {
					...
					fnRouteMatched = function(oEvent) {
						...
						var pWaitRebuildHistory;
						if (!history.state || history.state.feLevel === undefined) {
							var _sHash = HashChanger.getInstance().getHash();
							//navigating to the same hash will just rebuild the history but will not trigger a browser navigation
							pWaitRebuildHistory = oAppComponent.getRouterProxy().navToHash(_sHash);
						} else {
							pWaitRebuildHistory = Promise.resolve();
						}

						Promise.all([that.waitForRightMostViewReady(oEvent), pWaitRebuildHistory])
							.then(function(oRes) {
								var oView = oRes[0];
								var oAppComponent = CommonUtils.getAppComponent(oView);
								var oRouterProxy = oAppComponent.getRouterProxy();
								var oData = { oView: oView, oAppComponent: oAppComponent };
								oRouterProxy.computeTitleHierarchy(oData);
					...
					if (
						...
						if (sTarget || bNoHashChange) {
							if (!oUseContext || oUseContext.getBinding().isA("sap.ui.model.odata.v4.ODataListBinding")) {
								...
								var oMetaModel = oTargetControl.getModel().getMetaModel();
								var sEntitySet = oTargetControl.getComponentInstance().getEntitySet();
								var mParameters = { $$patchWithoutSideEffects: true, $$groupId: "$auto" };
								if (oMetaModel.getProperty("/" + sEntitySet + "/@com.sap.vocabularies.Common.v1.Messages")) {
									mParameters.$select = oMetaModel.getProperty(
										"/" + sEntitySet + "/@com.sap.vocabularies.Common.v1.Messages/$Path"
									);
								}
