		...
		var Extension = ControllerExtension.extend("sap.fe.core.controllerextensions.ContextManager", {
			...
			initializeFcl: function(oAppComponent, oFcl) {
				if (this.isFclEnabled(oAppComponent)) {
					...
					this.oRouterProxy = oAppComponent.getRouterProxy();
			...
			onStateChanged: function(oEvent) {
				...
				if (currentArguments !== undefined) {
					if (!currentArguments[sQueryKeyName]) {
						currentArguments[sQueryKeyName] = {};
					}
					currentArguments[sQueryKeyName].layout = oEvent.getParameter("layout");
				}
				this._updateUIstateForEachviews();
				...
				if (bIsNavigationArrow) {
					...
					this.oRouterProxy.navTo(currentRouteName, currentArguments);
				}
			},
			...
			onBeforeRouteMatched: function(oEvent) {
				var oQueryParams = oEvent.getParameters().arguments[sQueryKeyName];
			...
			handleFullScreen: function(oEvent) {
				...
				currentArguments[sQueryKeyName].layout = sNextLayout;
			...
			handleClose: function(oEvent) {
				...
				var iFclLevel = oView.getViewData().FCLLevel;
				var sColumnName = iFclLevel === 1 ? "midColumn" : "endColumn";
				var sNextLayout = this._getOwnerComponent()
					.getFcl()
					.getHelper()
					.getCurrentUIState().actionButtonsInfo[sColumnName].closeColumn;
				this.base.routing.navigateBackFromContext(oContext, { sLayout: sNextLayout });
			},
