...
sap.ui.define(
	...
	function(ResourceModel, CommonHelper, FieldBaseDelegate, AnnotationHelper, ManagedObject, Log, formatMessage, JSONModel) {
		"use strict";
		...
		var FieldHelper = {
			...
			_buildExpressionForTextProperty: function(sPropertyPath, oInterface) {
				var oMetaModel = oInterface.context.getModel(),
					...
					aPaths = sPath.split("/"),
					oTextAnnotation = oMetaModel.getObject(sPath + "@com.sap.vocabularies.Common.v1.Text"),
					sTextExpression = oTextAnnotation ? AnnotationHelper.value(oTextAnnotation, oInterface) : undefined,
					sExpression = "";
			...
			_formatProperty: function(oProperty, oInterface) {
				var oContext = oInterface.context,
					sPropertyPath = oContext && oContext.getPath(),
					aPaths = sPropertyPath && sPropertyPath.split("/"),
					oDataFieldValue;
				if (oProperty.$kind === "Property") {
					sPropertyPath = aPaths.length > 1 && aPaths.join("/").substr(aPaths[1].length + 2);
					oDataFieldValue = { $PropertyPath: sPropertyPath };
				} else {
					oDataFieldValue = oProperty;
				}
			...
			buildExpressionForProgressIndicatorPercentValue: function(sValue, sTarget, mUoM) {
				...
				sTarget = sTarget.charAt(0) === "{" ? "$" + sTarget : sTarget;
