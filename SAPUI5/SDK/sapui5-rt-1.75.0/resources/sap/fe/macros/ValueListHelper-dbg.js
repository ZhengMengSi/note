...
sap.ui.define(
	...
	function(jQuery, XMLTemplateProcessor, JSONModel, XMLPreprocessor, Fragment, MessageToast, InParameter, OutParameter, Log) {
		"use strict";
		var waitForPromise = {};
		...
		var ValueListHelper = {
			...
			createValueHelpDialog: function(propertyPath, oFVH, oTable, oFilterBar, oValueListInfo, bSuggestion) {
				...
				if (!oTable) {
					if (sFVHClass.indexOf("FieldValueHelp") > -1) {
						...
						oFVH.setFilterFields(entityIsSearchable() ? "$search" : "");
					}
				}
			...
			showValueListInfo: function(propertyPath, oFVH, bSuggestion, sConditionModel) {
				...
				if (waitForPromise[sWrapperId] || bExists) {
					...
					ValueListHelper.getValueListInfo(oFVH, oMetaModel, propertyPath, sConditionModel)
