...
sap.ui.define(
	...
	function(JSONModel, Log, ManagedObject, ResourceModel, XMLPreprocessor, SyncPromise, TraceInfo) {
		...
		function validateMacroSignature(sName, oMetadata, mContexts, oNode) {
			...
			aProperties.forEach(function(sKey) {
				...
				if (!oNode.hasAttribute(sKey)) {
					if (oPropertySettings.hasOwnProperty("defaultValue")) {
						oNode.setAttribute(sKey, oPropertySettings.defaultValue);
					} else if (oPropertySettings.required) {
		...
		function getAttributeValue(oNode, sKeyValue) {
			return function() {
				return wrapOutput(sKeyValue)(oNode.getAttribute(sKeyValue));
			};
		}
				...
				for (j = 0; j < aDefinitionPropertiesKeys.length; j++) {
					...
					if (oNode.hasAttribute(sKeyValue)) {
						aAttributeVisitorPromises.push(
							oVisitor.visitAttribute(oNode, oNode.attributes[sKeyValue]).then(getAttributeValue(oNode, sKeyValue))
						);
					} else {
						var oObj = {};
				...
				.then(function(aProps) {
					if (aProps != null) {
						...
						if (oMacroDefinition.create) {
							oProps = oMacroDefinition.create(oProps);
						}
