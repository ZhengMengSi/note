	 ...
	 return fnCreateRequestUrl(oBinding,sPath, null, aParams);
		...
		*/
			...
			handlers: {
				...
				onDataFieldForIntentBasedNavigation: function(oController, sSemanticObject, sAction, sMappings, vContext) {
				...
				fetchDefaultDisplayCurrency: function() {
					var oView = this.getView();
					...
					oAppComponent.getService("navigation").then(function(oNavigationService) {
						...
						oParseNavigationPromise.done(function(oAppData, oStartupParameters, sNavType) {
							var sEntitySet = that.routing.getEntitySet(oAppComponent);
							var aMandatoryFilterFields = oAppComponent
								.getMetaModel()
								.getObject("/" + sEntitySet + "@Org.OData.Capabilities.V1.FilterRestrictions/RequiredProperties");
							for (var i = 0; i < aMandatoryFilterFields.length; i++) {
								if (aMandatoryFilterFields[i].$PropertyPath === "DisplayCurrency") {
									if (!oAppData.oSelectionVariant.getSelectOption("DisplayCurrency").length) {
										if (oAppData.oDefaultedSelectionVariant.getSelectOption("DisplayCurrency").length) {
											var displayCurrencySelectOption = oAppData.oDefaultedSelectionVariant.getSelectOption(
												"DisplayCurrency"
											)[0];
