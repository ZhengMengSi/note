 ...
 * @copyright@
 */
sap.ui.define(["sap/base/Log", "sap/fe/macros/CommonHelper", "sap/fe/navigation/SelectionVariant", "sap/fe/navigation/NavError"], function(
	Log,
	CommonHelper,
	SelectionVariant,
	NavError
		...
		addConditionsToSelectionVariant: function(oSelectionVariant, mConditions) {
			...
			for (var sPropertyName in mConditions) {
				if (mConditions.hasOwnProperty(sPropertyName) && mConditions[sPropertyName].length) {
					...
					var aSelectOptions = mConditions[sPropertyName].map(fnCreateSelectOption).filter(function(oCondition) {
						return !!oCondition;
					});
					if (aSelectOptions.length) {
