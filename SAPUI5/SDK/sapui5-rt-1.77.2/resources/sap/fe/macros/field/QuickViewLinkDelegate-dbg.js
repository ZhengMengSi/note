	...
	function(
		...
		SimpleLinkDelegate.modifyLinkItems = function(oPayload, oBindingContext, aLinkItems) {
			if (aLinkItems.length !== 0) {
				...
				var mLineContextData = oLink.getBindingContext().getObject();
				var mPageContextData;
				if (oView.getAggregation("content")[0] && oView.getAggregation("content")[0].getBindingContext()) {
					mPageContextData = NavigationHelper.removeSensitiveData(oView.getAggregation("content")[0].getBindingContext());
				} else {
					var oController = oView.getController();
					mPageContextData = oController.filterBarConditions;
				}
				if (mPageContextData) {
					delete mPageContextData["@odata.context"];
				}
				var oMixedContext = merge({}, mPageContextData, mLineContextData);
				var oSelectionVariant = NavigationHelper.mixAttributesAndSelectionVariant(oMixedContext, new SelectionVariant());
				oAppComponent.getService("navigation").then(function(oNavigationService) {
					...
					var sEntitySet = NavigationHelper._getTargetCollection(oBindingContext);
					var oAppState = oNavigationService.storeInnerAppStateWithImmediateReturn(oSelectionVariant, true, sEntitySet, true);
					...
					aLinkItems.forEach(function(oLink) {
						...
						oNewShellHash = {
							...
							params: oParams,
							appStateKey: oAppState.appStateKey
						};
