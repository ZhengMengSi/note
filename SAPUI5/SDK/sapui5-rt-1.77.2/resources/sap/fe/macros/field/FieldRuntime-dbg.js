...
sap.ui.define(
	...
	function(XMLTemplateProcessor, XMLPreprocessor, Fragment, ResourceModel, SideEffectsUtil, Log) {
		...
		/**
		 * Checks the SideEffectsQueue for empty NavigationPropertyPaths and changes them to
		 * PropertyPaths with path "*"
		 * @function
		 * @name _checkPropertyPaths
		 * @param {Object} oSideEffectsRequestQueue the SideEffectsRequestQueue before they are requested
		 * @returns {Array} updated SideEffectsQueue, undefined if no SideEffectQueue provided
		 */
		function _checkPropertyPaths(oSideEffectsRequestQueue) {
			if (!oSideEffectsRequestQueue) {
				return undefined;
			}
			Object.keys(oSideEffectsRequestQueue).forEach(function(sContextPath) {
				var oSideEffect = oSideEffectsRequestQueue[sContextPath];
				oSideEffect.pathExpressions = SideEffectsUtil.replaceEmptyNavigationPaths(oSideEffect.pathExpressions);
			});
			return oSideEffectsRequestQueue;
		}
		...
		var FieldRuntime = {
			 ...
			 */
			onDataFieldWithNavigationPath: function(oController, sSemanticObjectName) {
			...
			requestSideEffects: function() {
				...
				oSideEffectsRequestQueue = _checkPropertyPaths(oSideEffectsRequestQueue) || {};
				Object.keys(oSideEffectsRequestQueue).forEach(function(sPath) {
