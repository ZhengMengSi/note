...
sap.ui.define(
	...
	function(ManagedObject, XMLPreprocessor) {
		...
		var traceNamespace = "http://schemas.sap.com/sapui5/extension/sap.fe.info/1",
			 ...
			 */
			aNamespaces = ["sap.m", "sap.ui.mdc", "sap.uxap", "sap.ui.table"],
			oCallbacks = {};
		...
		function resolve(oNode, oVisitor) {
			...
			if (bIsControl) {
				...
				return collectInfo(oNode, oVisitor)
					.then(function(result) {
						var bRelevant = Object.keys(result.properties).length + Object.keys(result.contexts).length > 0;
						if (bRelevant) {
		...
		function traceMacroCalls(sName, oMetadata, mContexts, oNode, oVisitor) {
			...
			if (aProperties.length === 0) {
				//In case the macro has not metadata.js we take all properties
				aProperties =
					Array.from(oNode.attributes)
						.map(function(attr) {
							return attr.name;
						})
						.filter(function(name) {
							return name !== "metadataContexts" && name.indexOf("xmlns:") < 0;
						}) || [];
			}
