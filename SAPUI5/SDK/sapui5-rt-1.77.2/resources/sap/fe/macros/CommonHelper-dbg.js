...
sap.ui.define(
	...
	function(
		...
		var Helper = {
			...
			/**
			 * Method to loop through the Line Contexts and merging the contexts
			 * @function
			 * @name getMergedLineContexts
			 * @memberof sap.fe.macros.CommonHelper
			 * @param  {Array} aLineContexts Array of all the Line Contexts
			 * @param  {object} oFilterBarSV Object of the FilterBar selection Variant
			 * @param  {Object} oPageContext Object containing FieldControl Type for a Field
			 * @returns {object} Selection Variant Object
			 */
			getMergedLineContexts: function(aLineContexts, oFilterBarSV, oPageContext) {
				var oSelectionVariant;
				oSelectionVariant = oFilterBarSV
					? NavigationHelper.mixAttributesAndSelectionVariant({}, oFilterBarSV.toJSONString())
					: NavigationHelper.mixAttributesAndSelectionVariant(oPageContext, new SelectionVariant());
				for (var i = 0; i < aLineContexts.length; i++) {
					if (oFilterBarSV) {
						oSelectionVariant = NavigationHelper.mixAttributesAndSelectionVariant(
							aLineContexts[i].getObject(),
							oSelectionVariant.toJSONString()
						);
						oSelectionVariant = NavigationHelper.removeSensitiveData(aLineContexts[i], oSelectionVariant);
					} else if (oPageContext) {
						oSelectionVariant = NavigationHelper.mixAttributesAndSelectionVariant(
							aLineContexts[i],
							oSelectionVariant.toJSONString()
						);
				...
				if (bDisplayOnly) {
					if (!bCanCreateProperty) {
						...
						if (sCreateMode && sCreateMode.indexOf("$") === 0) {
							...
							return "{= " + (bReturnBoolean ? "${ui>/editable} === 'Editable'" : sCheckUiEditMode) + "}";
						} else {
							return bReturnBoolean ? false : "{= " + sDisplayOrReadOnly + "}";
						}
					}
