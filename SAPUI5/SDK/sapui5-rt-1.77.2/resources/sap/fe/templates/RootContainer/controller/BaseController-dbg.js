			...
			computeTitleHierarchy: function(oData) {
				...
				if (oContext) {
					...
					if (oCurrentPage && oCurrentPage.getPageTitleInformation) {
						...
						aPathParts.forEach(function(sPathPart, i) {
							if (i === 0) {
								...
								if (sTargetType === "sap.fe.templates.ListReport") {
									var sAppTitle = oAppComponent.getMetadata().getManifestEntry("sap.app").title || "",
						...
						var oPageTitleInformationPromise = oCurrentPage.getPageTitleInformation().then(function(oPageTitleInformation) {
