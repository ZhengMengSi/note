...
sap.ui.define(
	...
	function(Log, ODataModelAnnotationHelper, CommonHelper, VariantManagement, mdcLibrary) {
		"use strict";
		...
		var AnnotationHelper = {
			...
			getFilterBarP13nMode: function(oViewData) {
				var aPersonalization = [],
					vPersonalization = oViewData.variantManagement && oViewData.variantManagement !== VariantManagement.None;
			...
			getTableChevronPressExpression: function(oNavigation, sNavigationOrCollectionPath, oDraft) {
				...
				if (oNavigation) {
					...
					if (oNavigationOutBound && oNavigationOutBound["display"] && oNavigationOutBound["display"]["target"]) {
						...
						return (
							".handlers.onChevronPressNavigateOutBound( $controller ,'" +
							sNavigationTarget +
						...
						return (
							...
							sNavigationOrCollectionPath +
							"', editable : " +
							(oDraft ? "!${$parameters>bindingContext}.getProperty('IsActiveEntity')" : "undefined") +
							"})"
						);
					} else {
						return undefined;
			...
			getTableRowActionExpression: function(oNavigation, sNavigationOrCollectionPath) {
				var oNavigationOutBound = oNavigation && oNavigation[sNavigationOrCollectionPath];
				if (
					(oNavigationOutBound && oNavigationOutBound["display"] && oNavigationOutBound["display"]["target"]) ||
					(oNavigationOutBound && oNavigationOutBound["detail"] && oNavigationOutBound["detail"]["outbound"]) ||
					(oNavigationOutBound && oNavigationOutBound["detail"]["route"])
				) {
					return "Navigation";
				} else {
					return undefined;
			...
			/**
			 * Returns the annotation path to use as presentation,
			 * annotation can be a presentation variant (with Line Item as visualization) or a Line Item
			 * @function
			 * @static
			 * @name sap.fe.core.AnnotationHelper.getCompliantAnnotationPath
			 * @memberof sap.fe.core.AnnotationHelper
			 * @param {object} oContext - context object for the presentation
			 * @returns {string} - Correct annotation path for the presentation
			 * @private
			 * @sap-restricted
			 **/
			getCompliantAnnotationPath: function(oContext) {
				var sPath = oContext.getPath(),
					sDefaultLineItemPath = ODataModelAnnotationHelper.getNavigationPath(sPath) + "/@com.sap.vocabularies.UI.v1.LineItem";
			...
			getNavigationInsertableRestrictions: function(
				...
				if (bInsertable && bInsertable.$Path && bInsertable.$Path.indexOf("/") > -1) {
					...
					return "{=  ${ui>/editable} === 'Editable' && ${" + sCurrentCollectionName + bInsertable.$Path + "}}";
				}
