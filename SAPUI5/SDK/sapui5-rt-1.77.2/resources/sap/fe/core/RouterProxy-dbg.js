	...
	function(Log, BaseObject, AppLifeCycle, HashChanger, EventProvider, Synchronization) {
		...
		return BaseObject.extend("sap.fe.core.RouterProxy", {
			...
			init: function(oAppComponent) {
				...
				this._initRoutesInfo(oAppComponent);
			...
			/**
			 * Builds an internal array of route names that don't accept an iAppState parameter
			 * @param {*} oAppComponent
			 */
			_initRoutesInfo: function(oAppComponent) {
				var oManifest = oAppComponent.getManifest(),
					aRoutes = oManifest["sap.ui5"].routing.routes,
					aTargets = oManifest["sap.ui5"].routing.targets,
					that = this;

				this._aRoutesWithoutAppState = [];
				aRoutes.forEach(function(oRoute) {
					// If we have an array of targets, then we're in FCL and all routes accept iAppState
					if (!Array.isArray(oRoute.target)) {
						var oTarget = aTargets[oRoute.target];
						if (oTarget.name !== "sap.fe.templates.ListReport") {
							that._aRoutesWithoutAppState.push(oRoute.name);
						}
					}
				});
			},
			...
			_internalNavToHash: function(sHash, bPreserveHistory) {
				...
				var oRouteInfo = this._oRouter.getRouteInfoByHash(sHash);
				if (
					this.sIAppStateKey &&
					!this._findAppStateInHash(sHash) &&
					oRouteInfo &&
					this._aRoutesWithoutAppState.indexOf(oRouteInfo.name) === -1
				) {
			...
			_pushNewState: function(oNewState, bPreserveHistory) {
				...
				var sAppStateKey = this._findAppStateInHash(oNewState.hash);
				if (sAppStateKey && this.sIAppStateKey !== sAppStateKey) {
					// We have a new app state --> replace in the whole history
					this.sIAppStateKey = sAppStateKey;

					this._oManagedHistory.forEach(function(oManagedState) {
						oManagedState.hash = that._setAppStateInHash(oManagedState.hash, sAppStateKey);
					});
				}
