...
sap.ui.define([
	...
	DateRangeType._IntFromToOnChangeHandler = function(sValue, oInstance) {
		if (this.basicLanguageText.indexOf("{0}") >= 0 && this.basicLanguageText.indexOf("{1}") >= 0) {
			var rx = new RegExp(this.basicLanguageText.replace("/", "\\/").replace("-{0}", "(\\-([a-zA-Z0-9_]+))").replace("+{1}", "(\\+([a-zA-Z0-9_]+))") + "$", 'i');
			if (sValue.match(rx)) {
				var sValueFrom = sValue.match(rx)[2];
				var sValueTo = sValue.match(rx)[4];
				if (sValueFrom && sValueTo) {
					var iValueFrom = parseInt(sValueFrom);
					var iValueTo = parseInt(sValueTo);

					if (!isNaN(sValueFrom) && sValueFrom <= oInstance._maxIntValue
					&& !isNaN(sValueTo) && sValueTo <= oInstance._maxIntValue) {
						oInstance.oModel.setProperty("/condition/operation", this.key);
	...
	DateRangeType._IntFromToFilterSuggestItem = function(sValue, oItem, oInstance) {
		...
		var isValidNumber = function(sValue) {
			return !!sValue.match(/(?!(0))^(\-|\+)([0-9]+)$/);
		}, iValueFrom, iValueTo;

		if (isValidNumber(aParts[1])) {
			iValueFrom = Math.abs(aParts[1]);
		}

		if (isValidNumber(aParts[3])) {
			iValueTo =  Math.abs(aParts[3]);
		}

		if (!iValueFrom || !iValueTo || iValueFrom > oInstance._maxIntValue || iValueTo > oInstance._maxIntValue) {
			oItem.setAdditionalText(null);
			oItem.setText(this.languageText);
			oItem._value1 = null;
			oItem._value2 = null;
		...
		var oDateFormatter = oInstance._getDateFormatter(true),
			oDateFrom, oDateTo;
		oDateFrom = new UniversalDateUtils.getRange(-iValueFrom, "DAY")[0];
		oDateTo =  new UniversalDateUtils.getRange(iValueTo, "DAY")[1];
	...
	DateRangeType.prototype.getFilterRanges = function() {
		...
		if (oCondition.operation === "LASTDAYS") {
			...
			var iValueFrom = parseInt(oCondition.value1),
				iValueTo = parseInt(oCondition.value2);
			oCondition.value1 = new UniversalDateUtils.getRange(-iValueFrom, "DAY")[0];
			oCondition.value2 =  new UniversalDateUtils.getRange(iValueTo, "DAY")[1];
		} else {
