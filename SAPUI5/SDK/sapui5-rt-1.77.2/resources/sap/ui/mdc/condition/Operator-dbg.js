...
sap.ui.define([
	...
	'./Condition'
		 ...
		 * @param {boolean} [oConfiguration.showInSuggest] If not set, to false the operator appears in the <code>DefineConditionPanel</code> // TODO better API
		 ...
		 * @param {boolean} [oConfiguration.checkInput] If set, the user input for this operator needs to be checked using a field help
		 * @constructor
		 * @private
		...
		var Operator = BaseObject.extend("sap.ui.mdc.condition.Operator", /** @lends sap.ui.mdc.condition.Operator.prototype */ {
			constructor: function(oConfiguration) {
				...
				this.checkInput = !!oConfiguration.checkInput; // to have always a boolean value
			}
		});
		 ...
		 * @public
		 */
		Operator.prototype.format = function(oCondition, oType, sDisplay) { // sDisplay needed in EEQ formatter
		...
		Operator.prototype.getCondition = function(sText, oType, sDisplayFormat, bDefaultOperator) {
			...
			if (this.test(sText) || (bDefaultOperator && sText)) {
				...
				if (aValues.length == this.valueTypes.length || this.valueTypes[0] === Operator.ValueType.Static
						|| (aValues.length === 1 && this.valueTypes.length === 2 && !this.valueTypes[1])) { // EEQ also valid without description
