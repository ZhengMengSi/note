	...
	var mControlTypes = {
			...
			Date: {
				...
				editOperator: {
					...
					"EEQ": {name: "sap/m/DatePicker", create: _createDatePickerControl},
			...
			Time: {
				...
				editOperator: {
					...
					"EEQ": {name: "sap/m/TimePicker", create: _createDatePickerControl}
				},
				createEdit: _createInputControl,
				createEditMulti: _createMultiInputControl, // FieldHelp needed to enter date
				createDisplay: _createTextControl,
			...
			DateTime: {
				...
				editOperator: {
					...
					"EEQ": {name: "sap/m/DateTimePicker", create: _createDatePickerControl}
				},
				createEdit: _createInputControl,
				createEditMulti: _createMultiInputControl, // FieldHelp needed to enter date
				createDisplay: _createTextControl,
...
//			}
	...
	function _createDefaultFieldHelp(sType) {
		...
		if (!oFieldHelp) {
			...
			if (FieldHelp) {
				oFieldHelp = new FieldHelp(mDefaultHelps[sType].id, mDefaultHelps[sType].properties);
...
//				this.addDependent(oFieldHelp); // TODO: where to add to control tree
	...
	function _handleContentLiveChange(oEvent) {
		...
		if (oFieldHelp && (!this._bIsMeasure || oSource.getShowValueHelp()) &&
				(aOperators.indexOf("EEQ") >= 0 || aOperators.indexOf("EQ") >= 0)) {
			...
			if (!this._fnLiveChangeTimer) {
				this._fnLiveChangeTimer = throttle(function() {
					...
					if (this._vLiveChangeValue) {
						// test EEQ operator first (==)
						var oOperator = FilterOperatorUtil.getEEQOperator();
						if (!oOperator.test(this._vLiveChangeValue)) {
							// test EQ operator
			...
			if (vOpenByTyping instanceof Promise) {
				vOpenByTyping.then(function() {
					...
					if (oFocusedElement && (containsOrEquals(this.getDomRef(), oFocusedElement))) {
