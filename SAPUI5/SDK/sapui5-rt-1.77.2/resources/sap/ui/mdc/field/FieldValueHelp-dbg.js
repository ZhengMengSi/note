...
//		_sDefaultDelegate: "sap/ui/mdc/field/FieldValueHelpDelegate" // TODO: better way to set default
		_sDefaultDelegate: "sap/ui/mdc/odata/v4/FieldValueHelpDelegate" // TODO: ude non-v4 delegate as default
	});
	...
	FieldValueHelp.prototype.invalidate = function(oOrigin) {
		...
		if (oOrigin) {
			...
			if (oWrapper && oOrigin === oWrapper) {
				...
				// --> invalidate rendering parent (Dialog or Popover)
				var oPopover = this.getAggregation("_popover");
				if (oDialog && oDialog.isOpen()) {
					oDialog.invalidate(oOrigin);
	...
	FieldValueHelp.prototype._getTextOrKey = function(vValue, bKey, oBindingContext, oInParameters, oOutParameters) {
		...
		if (oWrapper) {
			...
			if (bKey) {
				vResult = oWrapper.getTextForKey(vValue, _mapInParametersToHelp.call(this, oInParameters), _mapOutParametersToHelp.call(this, oOutParameters, true));
			} else {
				// use default in-parameters for check
				vResult = oWrapper.getKeyForText(vValue, _mapInParametersToHelp.call(this));
			}
