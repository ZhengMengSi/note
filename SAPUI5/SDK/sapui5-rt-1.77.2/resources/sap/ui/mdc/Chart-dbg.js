...
sap.ui.define([
	...
	function (
		...
		var ChartClass, SelectionHandler, DrillStackHandler, ChartTypeButton, MeasureItemClass;
		...
		Chart.prototype.applySettings = function (mSettings) {
			...
			this.oChartPromise = this.loadInnerChart().then(function () {

				if (!this._bIsBeingDestroyed) {

					return this.loadDelegateModule().then(function (oDelegate) {
		...
		function areModulesLoaded(aDependencies) {
			return aDependencies.every(function(sDependency) {

				// note: this variant of sap.ui.require("sap/lib") does not load the library
				return !!sap.ui.require(sDependency);
			});
		}

		Chart.prototype.loadInnerChart = function() {
			...
			if (areModulesLoaded(aModulePaths)) {
				return Promise.resolve();
			}

			return new Promise(function(resolve, reject) {

				sap.ui.require(aModulePaths, function(Chart, ChartTypeButtonLoaded, MeasureItem) {
					ChartClass = Chart;
		...
		Chart.prototype._rebind = function() {
			var oInnerChart = this.getAggregation("_chart"),
				oListBinding = oInnerChart && oInnerChart.getBinding("data");
