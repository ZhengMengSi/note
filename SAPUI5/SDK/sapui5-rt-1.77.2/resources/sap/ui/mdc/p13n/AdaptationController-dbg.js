...
sap.ui.define([
	...
	AdaptationController.prototype.createItemChanges = function(aNewItems){
		return this._executeAfterAsyncActions(function(){
			var oCurrentState = merge({}, this.getStateRetriever().call(this.getAdaptationControl(), this.oAdaptationControlDelegate));
			var aPreviousItems = oCurrentState.visibleFields;
			...
			var mExistingItems = aPreviousItems.reduce(function (mMap, oItem) {
			...
			aNewItems.forEach(function (oItem) {
				if (oItem.position > -1) {
					...
					if (mExistingItems[oItem.name]){//move if it exists
						var iOldPosition = mExistingItems[oItem.name].position;
						aNewItemsPrepared.splice(iNewPosition, 0, aNewItemsPrepared.splice(iOldPosition, 1)[0]);
					} else {//add if it does not exist
						aNewItemsPrepared.splice(iNewPosition, 0, oItem);
					}
				} else if (mExistingItems[oItem.name]) {//check if exists before delete
					aNewItemsPrepared[mExistingItems[oItem.name].position].position = -1;
				}
			});

			var fFilter = function (oItem) {
				return oItem.position > -1;
			};
	...
	AdaptationController.prototype._prepareAdaptationData = function(aPropertyInfo){
		...
		var aItemState = oControlState.visibleFields || [];
