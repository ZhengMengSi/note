...
sap.ui.define([
			...
			nodeInfo.forEach(function(node) {
				...
				if (!property || property !== "opacity") {
					...
					if (node.transform) {
						...
						newPosition = node.translate.slice();
						newRotation = node.rotate.slice();
						newScale = node.scale.slice();
					}
	...
	*/
	ViewStateManager.prototype.setVisibilityState = function(nodeRefs, visible, recursive) {
		...
		if (changed.length > 0) {
			...
			// annotations
			if (this._scene.getEnableDivAnnotations()) {
				var annotations = this._scene.getAnnotations();
				eventParameters.visible.forEach(function(visableNode) {
					var id = visableNode.userData.nodeId;
					var annotation = annotations.get(id);
					if (annotation) {
						annotation.setVisible(true);
					}
				});
				eventParameters.hidden.forEach(function(hiddenNode) {
					var id = hiddenNode.userData.nodeId;
					var annotation = annotations.get(id);
					if (annotation) {
	...
	ViewStateManager.prototype.setSelectionState = function(nodeRefs, selected, recursive, blockNotification) {
		...
		if (changed.length > 0) {
			...
			if (this._scene.getEnableDivAnnotations()) {
				var annotations = this._scene.getAnnotations();
				changed.forEach(function(nodeRef) {
					var id = nodeRef.userData.nodeId;
					var annotation = annotations.get(id);
					if (annotation) {
	...
	*/
	...
	ViewStateManager.prototype.getRestTransformationUsingJoint = function(nodeRef) {
		var joint = this._jointMap.get(nodeRef);
	...
	ViewStateManager.prototype._updateJointNode = function(joint) {
		if (!joint.node || !joint.parent) {
			return;
		}
		...
		while (parent) {
			var parentJoint = this._jointMap.get(parent);
			if (parentJoint) {
				if (!parentJoint.translation) {
					this._updateJoint(parentJoint);
				}
	...
	ViewStateManager.prototype._updateMaterialInNode = function(nodeInfo) {
		...
		if (node && node.children) {
			for (i = 0; i < node.children.length; i++) {
				...
				if (child.userData && child.userData.animatedColor) {
		...
		if (nodeInfo) {
			...
			nodeInfo.forEach(function(node) {
				...
				if (node.transform) {
					...
					nodeRef.position.set(node.translate[0], node.translate[1], node.translate[2]);
					nodeRef.quaternion.set(node.rotate[0], node.rotate[1], node.rotate[2], node.rotate[3]);
					nodeRef.scale.set(node.scale[0], node.scale[1], node.scale[2]);
				}
	 ...
	 * @param {any} nodeRef The node reference.
	 * @param {float[]} translation vector for postion, array of size 3, if null current rest translatrion is used
			...
			while (nodeRef) {
				for (var i = 0; i < nodeInfo.length; i++) {
					...
					if (node.transform) {
						...
						var position = new THREE.Vector3(node.translate[0], node.translate[1], node.translate[3]);
						var rotation = new THREE.Quaternion(node.rotate[0], node.rotate[1], node.rotate[2], node.rotate[3]);
						var scale = new THREE.Vector3(node.scale[0], node.scale[1], node.scale[3]);
			...
			nodeInfo.forEach(function(node) {
				...
				if (node.transform) {
					...
					if (forReversedPlaybacks) {
						var offsetTrans = this._getEndTranslationInPreviousSequence(nodeRef);
						if (offsetTrans) {
						...
						var offsetScale = this._getEndScaleInPreviousSequence(nodeRef);
						if (offsetScale) {
						...
						var offsetRotate = this._getEndRotateInPreviousSequence(nodeRef);
						if (offsetRotate) {
					...
					result = {};
					result.translation = node.translate.slice();
					result.quaternion = node.rotate.slice();
					result.scale = node.scale.slice();
				}
			}.bind(this));
		}
	...
	/**
	 * Get node tranlsation relative to rest position, defined by the last key of previous sequence.
	 *
	 * @function
	 * @name sap.ui.vk.ViewStateManager#_getEndTranslationInPreviousSequence
	 * @param {any} nodeRef node reference
	 * @param {sap.ui.vk.AnimationSequence} sequence current sequence
	 * @returns {float[]} translation
	 * @private
	 */
	ViewStateManager.prototype._getEndTranslationInPreviousSequence = function(nodeRef, sequence) {
		...
		for (var i = 0; i < playbacks.length; i++) {
			...
			if (!seq || seq._convertedFromAbsolute) {
				continue;
			}

			var nodeAnimation = seq.getNodeAnimation(nodeRef);
			if (!nodeAnimation) {
				continue;
			}
			var keyCount;
			var trans;
			var transTrack = nodeAnimation[AnimationTrackType.Translate];
			if (transTrack) {
				 keyCount = transTrack.getKeysCount();
				 if (keyCount) {
					 trans = transTrack.getKey(keyCount - 1).value;
				 }
			}
			if (trans) {
	...
	/**
	 * Get node scale relative to rest position, defined by the last key of previous sequence.
	 *
	 * @function
	 * @name sap.ui.vk.ViewStateManager#_getEndScaleInPreviousSequence
	 * @param {any} nodeRef node reference
	 * @param {sap.ui.vk.AnimationSequence} sequence current sequence
	 * @returns {float[]} scale
	 * @private
	 */
	ViewStateManager.prototype._getEndScaleInPreviousSequence = function(nodeRef, sequence) {
		...
		for (var i = 0; i < playbacks.length; i++) {
			...
			if (scaleTrack) {
				keyCount = scaleTrack.getKeysCount();
				if (keyCount) {
					sc = scaleTrack.getKey(keyCount - 1).value;
				}
			}
			if (sc) {
				scale[0] *= sc[0];
	 ...
	 *
	 * @function
	 * @name sap.ui.vk.ViewStateManager#_getEndOpacityInPreviousSequence
	 * @param {any} nodeRef node reference
	 * @param {sap.ui.vk.AnimationSequence} sequence current sequence
	 * @returns {float} opacity
	 * @private
	 */
	ViewStateManager.prototype._getEndOpacityInPreviousSequence = function(nodeRef, sequence) {
		...
		for (var i = 0; i < playbacks.length; i++) {
			...
			if (opacityTrack) {
				keyCount = opacityTrack.getKeysCount();
				if (keyCount) {
					op = opacityTrack.getKey(keyCount - 1).value;
				}
			}
			if (op) {
				opacity *= op;
	 ...
	 * @param {any} nodeRef node reference
	 * @param {sap.ui.vk.AnimationSequence} sequence current sequence
	 * @returns {float[]} quaternion of end position of last sequence
	 * @private
	 */
	ViewStateManager.prototype._getEndRotateInPreviousSequence = function(nodeRef, sequence) {
		if (sequence && sequence._convertedFromAbsolute) {
			return null;
		}
		...
		for (var i = 0; i < playbacks.length; i++) {
			...
			if (rotateTrack) {
				keyCount = rotateTrack.getKeysCount();
				if (keyCount) {
					ro = rotateTrack.getKey(keyCount - 1).value;
				}
			}
			if (ro) {
				var euler = new THREE.Euler(ro[0], ro[1], ro[2]);
				var quat = new THREE.Quaternion().setFromEuler(euler);
				prevQuat.premultiply(quat);
			}
		}

		return [ prevQuat.x, prevQuat.y, prevQuat.z, prevQuat.w ];
	};
	...
	ViewStateManager.prototype._getEndPropertyInPreviousSequence = function(nodeRef, property, sequence) {
		if (property === AnimationTrackType.Translate) {
			return this._getEndTranslationInPreviousSequence(nodeRef, sequence);
		} else if (property === AnimationTrackType.Scale) {
			return this._getEndScaleInPreviousSequence(nodeRef, sequence);
		} else if (property === AnimationTrackType.Rotate) {
			return this._getEndRotateInPreviousSequence(nodeRef, sequence);
		} else if (property === AnimationTrackType.Opacity) {
			return this._getEndOpacityInPreviousSequence(nodeRef, sequence);
		}else {
			return null;
		}
	};
		...
		if (nodeInfo) {
			...
			nodeInfo.forEach(function(node) {
				...
				if (node.transform) {
					...
					startT = node.translate.slice();
					startQ = new THREE.Quaternion(node.rotate[0], node.rotate[1], node.rotate[2], node.rotate[3]);
					startS = node.scale.slice();
				} else {
				...
				if (translateTrack && translateTrack.getIsAbsoluteValue()) {
					...
					for (j = 0; j < count; j++) {
						...
						translateTrack.updateKey(j, value);
					}
				...
				if (opacityTrack && opacityTrack.getIsAbsoluteValue()) {
					var restOpacity = this.getRestOpacity(node);
					count = opacityTrack.getKeysCount();
					for (j = 0; j < count; j++) {
						key = opacityTrack.getKey(j);
						value = key / restOpacity;
						opacityTrack.updateKey(j, value);
					}
	...
	ViewStateManager.prototype._setJointNodeOffsets = function(nodeRef) {
		...
		if (joint) {
			...
			if (joint.parent) {
				joint.parent.updateMatrixWorld();
				nodeRef.matrix.getInverse(joint.parent.matrixWorld).multiply(nodeRef.matrixWorld);
			} else {
				nodeRef.matrix.copy(nodeRef.matrixWorld);
			}

			nodeRef.matrix.decompose(nodeRef.position, nodeRef.quaternion, nodeRef.scale);
			var currentTranslation = nodeRef.position.toArray();
			...
			var aMatrix = new THREE.Matrix4();
			var rMatrix = new THREE.Matrix4();
			aMatrix.makeRotationFromQuaternion(nodeRef.quaternion);
			rMatrix.multiplyMatrices(aMatrix, invStartRMatrix);
			var offsetQ = new THREE.Quaternion().setFromRotationMatrix(rMatrix);
	...
	ViewStateManager.prototype.setTranslationKey = function(nodeRef, time, sequence) {
		...
		var offset = this._getEndTranslationInPreviousSequence(nodeRef, sequence);
		if (offset) {
			value[0] -= offset[0];
			value[1] -= offset[1];
			value[2] -= offset[2];
	...
	ViewStateManager.prototype.setScaleKey = function(nodeRef, time, sequence) {
		...
		var offset = this._getEndScaleInPreviousSequence(nodeRef, sequence);
		if (offset) {
			value[0] /= offset[0];
			value[1] /= offset[1];
			value[2] /= offset[2];
		...
		if (!track) {
			...
			sequence.setNodeAnimation(nodeRef, AnimationTrackType.Scale, track);
		} else {
			oldTrack = this._getTrackKeys(track);
		}
	...
	ViewStateManager.prototype.setRotationKey = function(nodeRef, time, euler, sequence) {
		...
		if (!track) {
			...
			sequence.setNodeAnimation(nodeRef, AnimationTrackType.Rotate, track);
		} else {
			oldTrack = this._getTrackKeys(track);
		}
		...
		var offset = this._getEndRotateInPreviousSequence(nodeRef, sequence);
		if (offset) {
	...
	ViewStateManager.prototype.setOpacityKey = function(nodeRef, time, sequence) {
		...
		var offsetOpacity = this._getEndOpacityInPreviousSequence(nodeRef, sequence);
		if (offsetOpacity) {
			value /= offsetOpacity;
		}
		...
		if (!track) {
			...
			sequence.setNodeAnimation(nodeRef, AnimationTrackType.Opacity, track);
		} else {
			oldTrack = this._getTrackKeys(track);
		}
