...
sap.ui.define([
	...
	SceneBuilder.prototype.setScene = function(info) {
		if (info.result !== 1) {
			...
			this._vkScene._setSceneBuilder(this);
			...
			this._resolve({
				...
				annotationViews: this._annotationViews,
	...
	*/
	...
	SceneBuilder.prototype.setViewAnnotations = function(command) {
		command.nodes.forEach(function(node) {
			if (node.contentType === NodeContentType.Annotation) {
				if (this._annotationViews.get(node.annotationId)) {
					this._annotationViews.get(node.annotationId).viewIds.set(command.viewId, true);
				} else {
					var viewIds = new Map();
					viewIds.set(command.viewId, true);
					this._annotationViews.set(node.annotationId, { viewIds: viewIds });
				}
			}
		}, this);
	};
	...
	SceneBuilder.prototype.createAnnotation = function(annotation, sceneId) {
		if (!annotation.position) {
			// create HTML annotations
			this._annotations.set(annotation.id, {
				annotation: annotation,
				node: this._nodes.get(annotation.nodeId),
				targetNode: (annotation.attachment && annotation.attachment.sid) ? this._nodes.get(annotation.attachment.sid) : null
			});
			return;
		}
