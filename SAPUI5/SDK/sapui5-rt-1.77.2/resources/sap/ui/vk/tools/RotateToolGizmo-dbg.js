...
sap.ui.define([
	...
	var getNearestRotaion = function(original, target) {
		...
		for (var ni = 0, nc = this._nodes.length; ni < nc; ni++) {
			...
			if (!nodeInfo.ignore) {
				...
				if (userData && userData.euler && userData.eulerInParentCoors) {
                    ...
                    userData.eulerInParentCoors.x = getNearestRotaion(userData.eulerInParentCoors.x, currentE.x);
                    userData.eulerInParentCoors.y = getNearestRotaion(userData.eulerInParentCoors.y, currentE.y);
					userData.eulerInParentCoors.z = getNearestRotaion(userData.eulerInParentCoors.z, currentE.z);
        ...
        for (var ni = 0, nc = this._nodes.length; ni < nc; ni++) {
			...
			if (!nodeInfo.ignore) {
                ...
                if (userData && userData.euler) {
                    ...
                    userData.euler.x = getNearestRotaion(userData.euler.x, currentE.x);
                    userData.euler.y = getNearestRotaion(userData.euler.y, currentE.y);
					userData.euler.z = getNearestRotaion(userData.euler.z, currentE.z);
                }
            }
        }
	};
