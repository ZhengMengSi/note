		...
		return new Promise(function(resolve) {
			if (!that._worker) {
				...
				if (sap.ui.Device.browser.internet_explorer) {
					...
					var polifillMisc = new URI(sap.ui.require.toUrl("sap/ui/vk/thirdparty/ie-polyfills.js"));
					if (polyfillPromise.is("relative")) {
						var base = new URI(location.href);
						polyfillPromise = polyfillPromise.absoluteTo(base);
						polyfillString = polyfillString.absoluteTo(base);
						polifillMisc = polifillMisc.absoluteTo(base);
					}
					scriptList = "'" + polyfillPromise.toString() + "','" + polyfillString.toString() + "','" + polifillMisc.toString() + "'," + scriptList;
				}
