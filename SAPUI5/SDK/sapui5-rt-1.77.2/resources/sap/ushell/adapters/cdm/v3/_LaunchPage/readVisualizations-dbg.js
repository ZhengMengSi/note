...
sap.ui.define([
     ...
     * @private
     */
    readVisualizations.getVizData = function (oSite, oVizReference) {
        var aCdmParts = this.getCdmParts(oSite, oVizReference);
        var oViz = this.get(oSite, readHome.getTileVizId(oVizReference)) || {};
        return {
            id: readHome.getTileId(oVizReference),
            vizId: readHome.getTileVizId(oVizReference) || "",
            vizType: this.getTypeId(oViz) || "",
            title: this.getTitle(aCdmParts) || "",
            subTitle: this.getSubTitle(aCdmParts) || "",
            icon: this.getIcon(aCdmParts) || "",
            keywords: this.getKeywords(aCdmParts) || [],
            info: this.getInfo(aCdmParts) || "",
            target: this.getTarget(oViz) || {}
        };
    };
