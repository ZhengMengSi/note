...
sap.ui.define([
     ...
     * @returns {Promise} Resolves into backend response page (in FLP style) after the page has been updated successfully
     *
     * @since 1.70.0
     * @protected
     */
    ...
    PagePersistence.prototype.getCatalogs = function (pageId, aRoleIds) {
        ...
        return this._fetchCatalogs(pageId).then(function (aRoles) {
            var aCatalogs = aRoles.reduce(function (catalogs, role) {
                if (bDoFilter && aRoleIds.indexOf(role.name) < 0) {
    ...
    PagePersistence.prototype._readPage = function (sPageId) {
        this._sCurrentPageId = null; // reset prevoiusly fetched catalog data
        this._catalogData = null;
        return new Promise(function (resolve, reject) {
            this._oODataModel.read("/pageSet('" + encodeURIComponent(sPageId) + "')", {
                urlParameters: { "$expand": "sections/tiles" },
                success: resolve,
                error: reject
            });
        }.bind(this));
    ...
    PagePersistence.prototype._convertODataToReferencePage = function (page) {
        ...
        if (oPage.sections && oPage.sections.results) {
            ...
            oPage.sections.forEach(function (section) {
                ...
                section.visualizations = section.tiles ? section.tiles.results : []; // naming mismatch between frontend and backend
                ...
                var iMax = section.visualizations.length;
                section.visualizations.sort(function (viz1, viz2) {
                    var i1 = viz1.itemIndex || iMax;
                    var i2 = viz2.itemIndex || iMax;
                    return i1 - i2;
                });
            });
        }
        ...
        var aSapErrorMessages = this.checkErrorMessage(oPage.id);
        oPage.editAllowed = aSapErrorMessages.length === 0;
        oPage.code = (aSapErrorMessages.length !== 0) ? aSapErrorMessages[0].code : "";
        oPage.message = (aSapErrorMessages.length !== 0) ? aSapErrorMessages[0].message : "";
        ...
        var oODataPage = {
            ...
            sections: (oPage.sections || []).map(function (section) {
                return {
                    id: section.id,
                    title: section.title,
                    tiles: (section.visualizations || []).map(function (tile, i) {
                        return {
                            id: tile.id,
     ...
     * @returns {Promise} A promise containing the catalog data in the form roles -> catalogs -> visualizaitons.
     *
     * @since 1.75.0
     * @private
     */
    PagePersistence.prototype._fetchCatalogs = function (pageId) {
        ...
        return new Promise(function (resolve, reject) {
            this._oODataModel.read("/pageSet('" + encodeURIComponent(pageId) + "')", {
                urlParameters: { "$expand": "roles/catalogs/visualizations" },
                    ...
                    this._catalogData.forEach(function (role) {
                        ...
                        role.catalogs.results.forEach(function (catalog) {
                            catalog.visualizations.results.forEach(function (visualization) {
                                role.visualizationIds.push(visualization.vizId);
                            });
                        });
                    });
    ...
    PagePersistence.prototype.getVizIds = function (aRoleIds) {
        ...
        if (this._catalogData) {
            this._catalogData.forEach(function (role) {
                if (!bFilter || aRoleIds.indexOf(role.name) > -1) {
