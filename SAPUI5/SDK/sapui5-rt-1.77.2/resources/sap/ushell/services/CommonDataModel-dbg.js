...
sap.ui.define([
    ...
    CommonDataModel.prototype._loadAndApplyPersonalization = function (oSite) {
        ...
        if (oCDMSiteVersion.compareTo("3.1.0") < 0) {
            this._oAdapter.getPersonalization(oCDMSiteVersion)
                .then(function (oPers) {
                    if (oPers) {
                        this._mPersonalizationDeltas = deepClone(oPers);
                    } else {
                        this._mPersonalizationDeltas = {version: "3.0.0", _version: "3.0.0"};
                    }
     ...
     * @returns {Promise} resolves when a valid personalised site was generated
     * @private
     * @since 1.76.0
     */
    ...
    /**
     * Helper function to apply a given personalization to a given page, convert it to CDM 3.1.0, and store the result.
     * Calls {@link _triggerMixinPersonalisationInSite}.
     * @see _triggerMixinPersonalisationInSite
     *
     * @param {object} site The CDM 3.0 site to apply the personalization and convert to CDM 3.1.0.
     * @param {object} personalization The personalization to be applied to the page.
     * @param {string} pageId The Page ID.
     * @returns {Promise<object>} Resolves to the personalized and converted page.
     *
     */
    CommonDataModel.prototype._triggerMixinPersonalisationInSiteAndStore = function (site, personalization, pageId) {
        return this._triggerMixinPersonalisationInSite(site, personalization)
            .then(function (oPersonalizedSite) {
                var oConvertedPersonalizedPage = this._oSiteConverter.convertTo("3.1.0", deepClone(oPersonalizedSite));
                if (this._oPersonalizedPages) {
                    this._oPersonalizedPages[pageId] = oConvertedPersonalizedPage;
                }
                return oConvertedPersonalizedPage;
            }.bind(this));
    };
            ...
            this._oGetSitePromise
                .done(function () {
                    if (bEnablePersonalization) {
                        this._getPageAndPersonalization(sPageId)
                            .then(function (oPageAndPersonalization) {
                                var oPage = oPageAndPersonalization.page,
                                    oPersonalization = oPageAndPersonalization.personalization,
                                    oConvertedPage = this._oSiteConverter.convertTo("3.0.0", deepClone(oPage));

                                this._oOriginalSite.pages[oPage.identification.id] = deepClone(oPage);
                                this._mPersonalizationDeltas = deepClone(oPersonalization);
                                var oPagePersonalization = oPersonalization[sPageId] || {};

                                this._triggerMixinPersonalisationInSiteAndStore.call(this, oConvertedPage, oPagePersonalization, sPageId)
                                    .then(resolve)
                                    .catch(function () {
                            ...
                            .catch(reject);
                    } else if (!this._oAdapter.getPage) {
                        resolve(this._oOriginalSite.pages[sPageId]);
                    } else {
                        this._oAdapter.getPage(sPageId)
                            .then(resolve)
                            .catch(reject);
                    }
                }.bind(this))
    ...
    /**
     * Gets the page and personalization object for a specified page id
     *
     * @param {string} sPageId The id of the required page
     * @returns {Promise<object>} Resolves with a object containing the page and personalization
     */
    CommonDataModel.prototype._getPageAndPersonalization = function (sPageId) {
        return new Promise(function (resolve, reject) {
            var getPagePromise;
            ...
            if (!this._oAdapter.getPage) {
                var oPage = this._oOriginalSite.pages[sPageId];
                getPagePromise = Promise.resolve(oPage);
            } else {
                getPagePromise = this._oAdapter.getPage(sPageId);
            }
            ...
            Promise
                ...
                .then(function (aPageAndPersonalization) {
                    resolve({
                        page: aPageAndPersonalization[0],
                        personalization: aPageAndPersonalization[1]
                    });
                })
                .catch(function () {
                    reject("CommonDataModel Service: Cannot get page " + sPageId + " or its personalization");
                });
        }.bind(this));
    };
     ...
     * @param {string[]} aPageId An array of page IDs to load
     *
     * @returns {Promise<object[]>}
     ...
     *
     */
    CommonDataModel.prototype.getPages = function (aPageId) {
        return new Promise(function (resolve, reject) {
                ...
                .done(function () {
                    if (!this._oAdapter.getPages) {
                        ...
                        for (var sPageId in this._oOriginalSite.pages) {
                            if (includes(aPageId, this._oOriginalSite.pages[sPageId].identification.id)) {
                                oPagesToReturn[sPageId] = this._oOriginalSite.pages[sPageId];
                            }
                        }
                        aPagesPromise = Promise.resolve(oPagesToReturn);
                    } else {
                        aPagesPromise = this._oAdapter.getPages(aPageId);
                    }

                    if (!bEnablePersonalization) {
                        return resolve(aPagesPromise.then(function (oPages) {
                            return Object.keys(oPages).map(function (sPageId) {
                                return oPages[sPageId];
                            });
                        }));
                    }

                    return aPagesPromise
                        .then(function (oPages) {

                            var aPromises = Promise.all(aPageId.map(function (sPageId) {
                                var oCurrentPage = oPages[sPageId];
                                if (!oCurrentPage) {
                                    // The given page id is unknown.
                                    return Promise.resolve();
                                }
                                return new Promise(function (resolveInner, rejectInner) {
                                    this._oAdapter.getPersonalization(this._oOriginalSite._version)
                                        .done(function (personalisation) {
                                            var oPage = oCurrentPage,
                                                oPersonalization = personalisation,
                                                oConvertedPage = this._oSiteConverter.convertTo("3.0.0", deepClone(oPage));

                                            this._oOriginalSite.pages[oPage.identification.id] = deepClone(oPage);
                                            this._mPersonalizationDeltas = deepClone(oPersonalization);
                                            var oPagePersonalization = oPersonalization[oPage.identification.id] || {};

                                            this._triggerMixinPersonalisationInSiteAndStore.call(this, oConvertedPage, oPagePersonalization, oPage.identification.id)
                                                .then(function (oPersPage) {
                                                    resolveInner(oPersPage);
                                                })
                                                .catch(rejectInner);
                                        }.bind(this))
                                        .fail(rejectInner);
                            ...
                            var oFilterPromise = aPromises.then(function (aPages) {
                                // Fitler all unknown page ids from the result.
                                return aPages.filter(function (oPage) {
                                    return !!oPage;
                                });
                            });
                            return resolve(oFilterPromise);
                        }.bind(this))
                        .catch(reject);
                }.bind(this))
                .fail(reject);
    ...
    /**
     * Returns all applications of the current Common Data Model.
     *
     * @returns {Promise<object>}
     *  A promise which resolves with all applications
     *  of the current Common Data Model
     *
     * @private
     * @since 1.75.0
     */
    ...
    /**
     * Returns all visualizations of the current Common Data Model.
     *
     * @returns {Promise<object>}
        ...
        this._oPersonalizationProcessor.extractPersonalization(oPersonalizedPage, oOriginalPage)
            .done(function (oExtractedPersonalization) {
                if (!isEmptyObject(oExtractedPersonalization)) {
                    ...
                    */
                    if (this._mPersonalizationDeltas.version === undefined || this._mPersonalizationDeltas._version === undefined) {
                        this._mPersonalizationDeltas.version = this._oOriginalSite._version;
                        this._mPersonalizationDeltas._version = this._oOriginalSite._version;
                    }
                    if (pageId) {
                        this._mPersonalizationDeltas[pageId] = oExtractedPersonalization;
                    } else {
                        this._mPersonalizationDeltas = oExtractedPersonalization;
                    }
                    this._setPersonalization(this._mPersonalizationDeltas)
