sap.ui.define(["sap/ui/core/mvc/OverrideExecution", "sap/suite/ui/generic/template/lib/TemplateAssembler",
	...
	"sap/suite/ui/generic/template/lib/testableHelper", "sap/suite/ui/generic/template/ListReport/semanticDateRangeTypeHelper", "sap/suite/ui/generic/template/ListReport/controller/staticChecksHelper"
], function(OverrideExecution, TemplateAssembler, ControllerImplementation, controllerFrameworkExtensions, testableHelper, semanticDateRangeTypeHelper, staticChecksHelper) {
	"use strict";
	...
	function getMethods(oComponent, oComponentUtils) {
		...
		return {
			...
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet) {
				function checkIfSmartChart(oEntityType, oTabItem) {
				...
				function getNormalizedVariantSetting(oSetting, oEntityType) {
					var oSettingNormalized = Object.assign({}, oSetting), sTableType;
					switch (oSettingNormalized && oSettingNormalized.tableSettings && oSettingNormalized.tableSettings.type) {
						case "AnalyticalTable" :
							sTableType = oEntityType["sap:semantics"] === "aggregate" ? "AnalyticalTable" : "GridTable";
							break;
						case "GridTable" :
						case "ResponsiveTable" :
						case "TreeTable" :
							sTableType = oSettingNormalized.tableSettings.type;
							break;
					    default:
					}
					oSettingNormalized.tableSettings = {
						type: sTableType,
						multiSelect: (oSetting && oSetting.tableSettings && oSetting.tableSettings.multiSelect) || false,
						inlineDelete: (oSetting && oSetting.tableSettings && oSetting.tableSettings.inlineDelete) || false,
						selectAll: (oSetting && oSetting.tableSettings && oSetting.tableSettings.selectAll) || false,
						selectionLimit: (oSetting && oSetting.tableSettings && oSetting.tableSettings.selectionLimit) || 200
					};
					if (oSettingNormalized.tableSettings.multiSelect && oSettingNormalized.tableSettings.inlineDelete) {
						throw new Error("Both inlineDelete and multiSelect options for table are not possible");
					}

					if (sTableType !== "ResponsiveTable" && oSettingNormalized.tableSettings.inlineDelete) {
						throw new Error("InlineDelete property is not supported for " + sTableType + " type table");
					}
					return oSettingNormalized;
				}
				var oLrSettings = Object.assign({}, oSettings), sTableType;
				var oLeadingEntitySet = oMetaModel.getODataEntitySet(sLeadingEntitySet);
				var oLeadingEntityType = oMetaModel.getODataEntityType(oLeadingEntitySet.entityType);
				if (Device.system.phone) {
					sTableType = "ResponsiveTable";
				} else {
					sTableType = (oLrSettings && oLrSettings.tableSettings && oLrSettings.tableSettings.type) || (oLeadingEntityType["sap:semantics"] === "aggregate" ? "AnalyticalTable" : "ResponsiveTable");
				}
				oLrSettings.tableSettings = oLrSettings.tableSettings || {};
				if (oLrSettings.quickVariantSelectionX) {
					...
					for (var sKey in oVariants) {
						var oEntitySet = oMetaModel.getODataEntitySet(oVariants[sKey].entitySet || sLeadingEntitySet);
						var oEntityType = oMetaModel.getODataEntityType(oEntitySet.entityType);
						oVariants[sKey].isSmartChart = checkIfSmartChart(oEntityType, oVariants[sKey]);
						if (!oVariants[sKey].isSmartChart) {
							oVariants[sKey].tableSettings = oVariants[sKey].tableSettings || oLrSettings.tableSettings;
							oVariants[sKey].tableSettings.type = (Device.system.phone ? "ResponsiveTable" : oVariants[sKey].tableSettings.type) || sTableType;
							oVariants[sKey] = getNormalizedVariantSetting(oVariants[sKey], oEntityType);
					...
					//for single  variant
					oLrSettings.isResponsiveTable = sTableType === "ResponsiveTable";
					oLrSettings.tableSettings.type = sTableType;
					oLrSettings = getNormalizedVariantSetting(oLrSettings, oLeadingEntityType);
				}
				if (oLeadingEntityType && oLeadingEntityType.property && oSettings && oSettings.createWithParameterDialog && oSettings.createWithParameterDialog.fields) {
					staticChecksHelper.checkErrorforCreateWithDialog(oLeadingEntityType, oSettings);
				}
				oLrSettings.allControlConfiguration = oLeadingEntityType["com.sap.vocabularies.UI.v1.SelectionFields"] ? oLeadingEntityType["com.sap.vocabularies.UI.v1.SelectionFields"].slice() : [];
				oLrSettings.datePropertiesSettings = semanticDateRangeTypeHelper.setSemanticDateRangeSettingsForDateProperties(oLrSettings, oLeadingEntityType);
				return oLrSettings;
			}
		};
	}
