	...
	function getMethods(oTemplateContract, oController, oCommonUtils) {
		...
		function getConfiguredPopoverIfNeeded(iSituation){
			return new Promise(function (fnResolve) {
				...
				oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.MessageInfluencingSave", {
		...
		function fnRestartActivation(fnOperationCallback, fnCancelCallback){
			...
			getConfiguredPopoverIfNeeded(3).then(function (oWarningDialog) {
				if (oWarningDialog) {
					oWarningDialog.open();
				} else {
					fnCancelCallback();
