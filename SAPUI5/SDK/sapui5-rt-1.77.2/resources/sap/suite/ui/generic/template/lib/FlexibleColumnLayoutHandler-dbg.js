	...
	function(BaseObject, FlexibleColumnLayoutSemanticHelper, fioriLibrary, extend) {
		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
			...
			function getAppStatesPromiseForNavigation(oCurrentIdentity, oTargetIdentity){
				var sTargetLayout = (oTargetIdentity.treeNode.fCLLevel === 1 && oCurrentIdentity.treeNode.parentRoute === oTargetIdentity.treeNode.sRouteName && oCurrentIdentity.treeNode.fCLLevel === 2) ? oUiState.actionButtonsInfo.endColumn.closeColumn : getTargetLayoutForTreeNode(oTargetIdentity.treeNode);
