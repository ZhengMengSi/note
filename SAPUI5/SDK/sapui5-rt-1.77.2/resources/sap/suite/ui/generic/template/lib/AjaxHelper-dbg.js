...
=	 * {
	...
	function parseXML (xmlString) {
		var response = {};
			var xmlDocument = new window.DOMParser().parseFromString(xmlString, "application/xml" );
			var paraserError = xmlDocument.getElementsByTagName("parsererror")[0];
			if (paraserError) {
				response.success = false;
				response.textStatus = RETURN_STATUS.PARSING_ERROR;
				response.error = paraserError.innerText;
			} else {
				response.data = xmlDocument;
			}
	return response;
	}
	/**
