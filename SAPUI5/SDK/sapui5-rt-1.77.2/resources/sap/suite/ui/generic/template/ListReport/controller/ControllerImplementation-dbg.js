...
sap.ui.define([
	...
	"sap/suite/ui/generic/template/ListReport/controller/CreateWithDialogHelper"
], function(Filter, ExtensionAPI, listUtils, IappStateHandler, MultipleViewsHandler,
		        WorklistHandler, ShareUtils, Log, ObjectPath, StableIdHelper, deepExtend, CreateWithDialogHelper) {
	"use strict";
	...
	return {
		getMethods: function(oViewProxy, oTemplateUtils, oController) {
			...
			function addEntry(sVariantKey) {
				...
				var oDialog = oController.byId(oState.oCreateWithDialogHelper.getStableIdForDialog());
				if (oDialog) {
					oTemplateUtils.oCommonUtils.executeIfControlReady(oState.oCreateWithDialogHelper.createWithDialog.bind(null), sIdForCreateButton);
				} else {
			...
			return {
				onInit: function() {
					...
					oState.oCreateWithDialogHelper = new CreateWithDialogHelper(oState, oController, oTemplateUtils);
				...
				handlers: {
					...
					contextEditableChangedForSmartField : function(oEvent) {
						var oSmartField = oEvent.getSource();
						if (!oSmartField.getContextEditable()) {
							oSmartField.setContextEditable(true);
						}
					},
					onCancelCreateWithPopUpDialog: function() {
						oState.oCreateWithDialogHelper.onCancelPopUpDialog();
					},
					onSaveCreateWithPopUpDialog : function(oEvent) {
						oState.oCreateWithDialogHelper.onSavePopUpDialog(oEvent);
					},
