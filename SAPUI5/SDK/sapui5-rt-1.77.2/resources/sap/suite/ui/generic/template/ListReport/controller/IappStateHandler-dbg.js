...
`		@param {object} App data
		...
		*/
		...
		function fnAdaptToAppState(oAppData, oURLParameters, sNavType){
			...
			if (oState.oWorklistData.bWorkListEnabled) {
				...
				if (oDataLoadSettings) {
					var sLoadBehaviour = oDataLoadSettings.loadDataOnAppLaunch;
				}
		...
		function isDataShown(sLoadBehaviour, bIsMandatoryFilterNotFilled) {
			var bDataLoad;
			var filtersWithValues = oState.oSmartFilterbar.getFiltersWithValues();
			var loadBehaviourMap = ["ifAnyFilterExist", "always", "never"];
			if (!loadBehaviourMap.includes(sLoadBehaviour)) {
				sLoadBehaviour = "ifAnyFilterExist";
			}
			if (sLoadBehaviour === "ifAnyFilterExist" || sLoadBehaviour === null || sLoadBehaviour === undefined) {
		...
		*/
		function onSFBVariantInitialise() {
			...
			var multiView;
			multiView = sLoadBehaviour === null || sLoadBehaviour === undefined ? oState.oMultipleViewsHandler.getEnableAutoBinding() : true;
