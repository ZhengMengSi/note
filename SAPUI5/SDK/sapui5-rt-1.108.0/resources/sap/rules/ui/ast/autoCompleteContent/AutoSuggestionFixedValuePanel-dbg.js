	...
	var autoSuggestionFixedValuePanel = Control.extend("sap.rules.ui.ast.autoCompleteContent.AutoSuggestionFixedValuePanel", {
		...
		_formatFilterParams: function (aFilters, valuehelpSourceType) {
			...
			if (aFilters[0] && aFilters.length === 1 && aFilters[0].aFilters[0] && aFilters[0].aFilters[0].sOperator) {
				aFilters = that._formatSingleFilter(aFilters, valuehelpSourceType);
				...
				aFilters = that._formatMultiFilter(aFilters, 0, true, valuehelpSourceType);
				aFilters = that._formatMultiFilter(aFilters, 1, true, valuehelpSourceType);
				...
				aFilters = that._formatMultiFilter(aFilters, 0, true, valuehelpSourceType);
				aFilters = that._formatMultiFilter(aFilters, 1, false, valuehelpSourceType);
				...
				aFilters = that._formatMultiFilter(aFilters, 0, false, valuehelpSourceType);
				aFilters = that._formatMultiFilter(aFilters, 1, true, valuehelpSourceType);
		...
		_formatSingleFilter: function (aFilters, valuehelpSourceType) {
			var that = this;
			var _valueField = null;

			for (var iterator = 0; iterator < aFilters[0].aFilters.length; iterator++) {
				if (aFilters[0].aFilters[iterator].sOperator === "Contains" && valuehelpSourceType === "U") {
					aFilters[0].aFilters[iterator].sOperator = "EQ";
				} else if (aFilters[0].aFilters[iterator].sOperator === "Contains") {
		...
		_formatMultiFilter: function (aFilters, index, bMultifilter, valuehelpSourceType) {
			var that = this;
			var _valueField = null;
			...
			for (var iterator = 0; iterator < aComputeFilterParma.length; iterator++) {
				if (aComputeFilterParma[iterator].sOperator === "Contains" && valuehelpSourceType === "U") {
					aComputeFilterParma[iterator].sOperator = "EQ";
				} else if (aComputeFilterParma[iterator].sOperator === "Contains") {
