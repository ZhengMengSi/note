...
sap.ui.define([
	...
	var VBox;
	var Button;
	var INTL_TIMEZONE = Intl.DateTimeFormat().resolvedOptions().timeZone;
	var UI_TIMEZONE = sap.ui.getCore().getConfiguration().getTimezone();
	...
	DateRangeType.prototype.initialize = function(oJson) {
		...
		if (oJson.operation === "DATERANGE") {
			...
			if (typeof oJson.value1 === "string") {
				var sValue1 = oJson.value1;
				oJson.value1 = oJson.value1 === "" ? null : (new UniversalDate(oJson.value1)).oDate;
				// When the date is in UTC (with "Z" at the end of the string), then it should be set with T00:00:00.000Z in local time too
				// and UI_TIMEZONE === INTL_TIMEZONE
				if (this._bSingleFilterRestriction && this.oDateFormat && this.oDateFormat.UTC && oJson.value1.getHours() !== 0 &&
					UI_TIMEZONE === INTL_TIMEZONE && sValue1.indexOf("T00:00:00.000Z") > 0){
					oJson.value1.setHours(0,0,0);
				}
			}
			oProperty.value1 = oJson.value1;
	...
	DateRangeType.prototype.getFilterRanges = function() {
		var oSecondDate,
			oCondition = Object.assign({}, this.getCondition()),
			aValues = [],
			oRange;

		if (oCondition.value1 && oCondition.value1 instanceof Date) {
			oCondition.value1 = new Date(oCondition.value1.valueOf());
		}

		if (oCondition.value2 && oCondition.value2 instanceof Date) {
			oCondition.value2 = new Date(oCondition.value2.valueOf());
		}

		if (this._bSingleFilterRestriction) {
			if (oCondition.operation === "DATE") {
				oCondition.value1 = oCondition.value1 && DateTimeUtil.localToUiTimezone(oCondition.value1);
			}
		...
		if (oCondition.operation === "DATE") {
			...
			oCondition.value1 = DateTimeUtil.localToUiTimezone(oCondition.value1);
			...
			if (oRange) {
				...
				if (oCondition.operation === "DATERANGE") {
					// TODO: check if value1 and value2 can be something different than date here.
					// This check is added because of failing test, but it looks like a wrong mock of the test
					// rather than a real scenario
					oCondition.value1 = oCondition.value1 instanceof Date ? DateTimeUtil.localToUiTimezone(oCondition.value1) : oCondition.value1;
					oCondition.value2 = oCondition.value2 instanceof Date ? DateTimeUtil.localToUiTimezone(oCondition.value2) : oCondition.value2;
				}
				oCondition.operation = "BT";
				// ensure the day and set time to beginning of day
				oCondition.value1 = DateRangeType.setStartTime(oCondition.value1).oDate;
	...
	DateRangeType.prototype._prepareDDRValues = function (oOperation, oValue, sOperationName) {
		...
		if (sOperationName === "DATE" || sOperationName === "DATERANGE") {
			return oValue.values;
		}
