...
sap.ui.define([
	...
	FilterController.prototype.getPanel = function(oPayload) {
		...
		return new Promise(function(resolve) {
			...
			sap.ui.require([
				...
				var sControlNumber, bUseQueryPanel = true; //this._getFilterQueryPanelParameter();
				...
				if (bUseQueryPanel) {
					...
					if (oSmartContainer) {
						...
						if (oSmartContainer.getParent().isA("sap.m.IconTabBar")) {
							sControlNumber = oSmartContainer.getParent().getSelectedKey();
						}
					}
	...
	FilterController.prototype._fieldChangeHandler = function(oEvent) {
		var key, value, aFilterData = [], oCondition,
			oFilterProvider = this.oFilterProvider,
			oFilterData = oFilterProvider.getFilledFilterData(this._aActiveFilterPanelFieldNames),
	...
	FilterController.prototype._updateFilterData = function(aData) {
		...
		if (aFilterItems && aFilterItems.length > 0) {
			for (i = 0; i < aFilterItems.length; i++) {
				oItem = aData  || (this.getControlDataInitial().filter.filterItems && this.getControlDataInitial().filter.filterItems.length > 0) ? Object.assign({}, aFilterItems[i]) : aFilterItems[i];
				...
				if (oItem.token) {
					...
					oFilterData[sName].ranges.push(aFilterItems[i]);
				}
			}
