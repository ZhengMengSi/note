sap.ui.define(["sap/ui/core/mvc/OverrideExecution",
	...
	filterSettingsPreparationHelper, StableIdHelper, AnnotationHelper, isEmptyObject, ALPAnnotationHelper) {
	"use strict";
		...
		return {
			...
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet, oInternalManifest, oModel){
				...
				if (window["sap-ushell-config"] && window["sap-ushell-config"].apps && window["sap-ushell-config"].apps.insights) {
					oAlpSettings.bInsightsEnabled =  window["sap-ushell-config"].apps.insights.enabled ? true : false;
				} else {
