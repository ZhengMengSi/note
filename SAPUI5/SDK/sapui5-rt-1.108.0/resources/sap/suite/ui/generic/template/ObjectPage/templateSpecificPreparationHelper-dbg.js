sap.ui.define([
	...
	*/
	function fnGetTemplateSpecificParameters(oComponentUtils, oMetaModel, oOriginalSettings, oDevice, sLeadingEntitySet, oInternalManifest) {
		...
		function fnGetDataFieldDetails(oDataField, sMetaModelPathContentPrefix, nIndex, oOpSettings) {
			...
			var oDataFieldAction = AHModel.format(oMetaModel.getContext(sMetaModelPathContentPrefix + nIndex + "/Action"));
			switch (oDataField.RecordType) {
				case "com.sap.vocabularies.UI.v1.DataFieldForAction":
					var aParameters = [];
					aParameters.push("'" + oDataFieldAction + "'");
					aParameters.push("'" + oDataField.Label.String + "'");
					...
					var oPageAnnotatedAction = oOpSettings.annotatedActions && oOpSettings.annotatedActions[oDataFieldAction.split("/")[1]];
		...
		function fnNormalizeSubSection(oSectionData, oSubSectionData){
			...
			oSubSectionData.blocks.forEach(function(oBlock){
				...
				oBlock.temporaryData.content.forEach(function(oDataField, iIndex){
					...
					var oActionDataField = fnGetDataFieldDetails(oDataField, sMetaModelPathContentPrefix, iIndex, oOpSettings);
