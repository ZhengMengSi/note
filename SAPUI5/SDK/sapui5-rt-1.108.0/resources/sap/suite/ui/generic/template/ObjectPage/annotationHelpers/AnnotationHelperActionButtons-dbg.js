...
sap.ui.define([
	...
	function getCallAction(oDataField) {
		var sAction = oDataField.Action.String;
		/* sAction and sInvocationGrouping are technical identifiers, so they should not contain any 'special' characters. However, sLabel is a string in 
		   natural language, which might contain the '. Therefore, this needs to be escaped by a preceeding \. */
		var sLabel = oDataField.Label.String.replace("'", "\\'");
		...
		return "._templateEventHandlers.onCallAction('" + sAction + "', '" + sLabel + "', '" + sInvocationGrouping + "', " + bIsCopyAction + ")";
	}
