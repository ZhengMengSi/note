sap.ui.define(["sap/ui/core/mvc/OverrideExecution",
			   ...
			   "sap/suite/ui/generic/template/ListReport/AnnotationHelper"
					...
					staticChecksHelper, preparationHelper, deepExtend, FeError, StableIdHelper, Context, AnnotationHelper, isEmptyObject, LRAnnotationHelper) {
	"use strict";
		...
		return {
			...
			*/
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet, oInternalManifest, oModel, oDraftContext) {
				...
				if (window["sap-ushell-config"] && window["sap-ushell-config"].apps && window["sap-ushell-config"].apps.insights) {
					oLrSettings.bInsightsEnabled =  window["sap-ushell-config"].apps.insights.enabled ? true : false;
				} else {
