sap.ui.define([
    ...
    return Controller.extend("sap.ovp.app.Main", {
        ...
        _processMacroFilters: function () {
            var oFBConditionsModel = this.getView().getModel("fbConditions");
            var oFilterConditions = {};
            if (oFBConditionsModel && oFBConditionsModel.getProperty("/filter")) {
                oFilterConditions = JSON.parse(oFBConditionsModel.getProperty("/filter"));
            }

            var aPaths = Object.keys(oFilterConditions) || [];
            var aFilters = aPaths.map(function(sPath) {
                var aCondition = oFilterConditions && oFilterConditions[sPath];
                var aConditionFilters = [];

                for (var i = 0; i < aCondition.length; i++) {
                    var oCondition = aCondition[i];
                    var oConditionFilter = new Filter({
                        path: sPath,
                        operator: oCondition.operator,
                        value1: oCondition.values[0],
                        value2: oCondition.values[1]
                    });
                    aConditionFilters.push(oConditionFilter);
                }
                return new Filter(aConditionFilters, /*bAnd*/ false);
            });

            this.aFilters = aFilters.length > 0 ? [new Filter(aFilters, /*bAnd*/ true)] : [];
        },
        ...
        onMacroFilterBarSearch: function (oEvent) {
            ...
            var oFBConditions = oEvent.getParameter("conditions") || {};

            for (var sPath in oFBConditions) {
                if (oFBConditions[sPath].length === 0) {
                    delete oFBConditions[sPath]; // Remove empty condition
                }
            }
            mFBConditions.setProperty("/filter", JSON.stringify(oFBConditions, null, "  "));
