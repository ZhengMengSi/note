...
export class BuildingBlockBase {
	...
	protected id!: string;
	constructor(oProps: Record<string, any>, _oControlConfig?: any, _oSettings?: any) {
		Object.keys(oProps).forEach((propName) => {
			this[propName as keyof this] = oProps[propName];
		});
	}
...
const RUNTIME_BLOCKS: Record<string, BuildingBlockDefinition> = {};
...
Fragment.registerType("FE_COMPONENTS", {
	...
	init: async function (mSettings: Record<string, any>) {
		...
		return (ManagedObject as unknown as ManagedObjectEx).runWithPreprocessors(
			() => {
				const renderedControl = new MyClass(classSettings).render(mSettings.containingView, appComponent);
