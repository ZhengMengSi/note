...
class TemplatedViewService extends Service<TemplatedViewServiceSettings> {
	...
	init() {
		...
		let sPageModelCacheKey: string | undefined;
	...
	_getCachedModel(sCacheKey: string | undefined) {
		if (sCacheKey && Core.getConfiguration().getViewCache()) {
			return CacheManager.get(sCacheKey);
		}
		return undefined;
	}
	_setCachedModel(sCacheKey: string | undefined, oCacheModel: any) {
		if (sCacheKey && Core.getConfiguration().getViewCache()) {
	...
	async createView(
		...
		sStableId: any,
		sCacheKey: any,
		sPageModelCacheKey: string | undefined,
		oPageModelCache: any,
		TemplateConverter: any,
		MetaModelConverter: any
	): Promise<any | void> {
