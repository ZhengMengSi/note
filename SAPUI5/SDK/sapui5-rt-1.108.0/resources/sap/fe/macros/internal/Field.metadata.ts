...
const Field = MacroMetadata.extend("sap.fe.macros.internal.Field", {
	...
	setUpDisplayStyle: function (this: FieldType, oProps: FieldProperties, oDataField: any, oDataModelPath: DataModelObjectPath): void {
		...
		if (oDataModelPath?.targetEntityType?.annotations?.Common?.SemanticKey) {
			const aSemanticKeys = oDataModelPath.contextLocation?.targetEntityType?.annotations?.Common?.SemanticKey;
			...
			if (bIsSemanticKey && oProps.formatOptions.semanticKeyStyle) {
				...
				oProps.displayStyle =
					oProps.formatOptions.semanticKeyStyle === "ObjectIdentifier" ? "ObjectIdentifier" : "LabelSemanticKey";
				return;
			}
		}
