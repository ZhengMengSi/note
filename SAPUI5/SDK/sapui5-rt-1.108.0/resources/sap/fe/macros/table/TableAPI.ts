...
class TableAPI extends MacroAPI {
	...
	onBeforeExport(oEvent: UI5Event) {
		const isSplitMode = (oEvent.getParameters() as any).userExportSettings.splitCells,
			...
			oExportColumns = (oEvent.getParameters() as any).exportSettings.workbook?.columns,
	...
	static updateExportSettings(oExportColumns: any, oColumns: TableColumn[], oTableController: PageController, isSplitMode: boolean): any {
		let refColumn: any = null;
		let additionalPropertyIndex: number;
		oExportColumns.forEach(function (oColumnExport: any) {
			oColumnExport.label = DelegateUtil.getLocalizedText(oColumnExport.label, oTableController);
			...
			oColumns?.forEach((column) => {
				...
				if (isSplitMode) {
					...
					const regex = /(.*)-additionalProperty(\d+)/.exec(oColumnExport.columnId);
					if (regex === null) {
						additionalPropertyIndex = 1;
						refColumn = oColumnExport;
					} else if (regex[1] === refColumn?.columnId) {
						oColumnExport.label =
							oColumnExport.label === refColumn.label
