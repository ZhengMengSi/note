...
import type { CustomBasedTableColumn } from "sap/fe/core/converters/controls/Common/Table";
...
export default Object.assign({}, TableDelegateBase, {
	...
	_computeLabel: function (oProperty: any, labelMap: { [label: string]: any[] }) {
		const propertiesWithSameLabel = labelMap[oProperty.label];
		if (propertiesWithSameLabel?.length > 1 && oProperty.path?.includes("/")) {
			oProperty.label = oProperty.label + " (" + oProperty.additionalLabels?.join(" / ") + ")";
		}
		delete oProperty.additionalLabels;
	},
	//Update VisualSetting for columnWidth calculation and labels on navigation properties
	_updatePropertyInfo: function (oTable: Table, aProperties: any) {
		const labelMap: { [label: string]: any[] } = {};
		aProperties.forEach((oProperty: any) => {
			if (!oProperty.propertyInfos) {
				// Only for non-complex properties
				labelMap[oProperty.label] =
					labelMap[oProperty.label] !== undefined ? labelMap[oProperty.label].concat([oProperty]) : [oProperty];
			}
		});
		aProperties.forEach((oProperty: any) => {
			this._computeVisualSettingsForFieldGroup(oTable, oProperty, aProperties);
			this._computeVisualSettingsForPropertyWithValueHelp(oTable, oProperty);
			// bcp: 2270003577
			...
			this._computeLabel(oProperty, labelMap);
		});
		return aProperties;
	},
	...
	_isFilterableNavigationProperty: function (oColumnInfo: any, oMetaModel: any) {
		...
		const property = oMetaModel.getObject(oColumnInfo.annotationPath.slice(0, oColumnInfo.annotationPath.lastIndexOf("/")));
	...
	_fetchPropertyInfo: function (oMetaModel: any, oColumnInfo: any, oTable: any, oAppComponent: any, bUseAdditionalProperties: any) {
		...
		if (oColumnInfo.propertyInfos && oColumnInfo.propertyInfos.length) {
			...
			oPropertyInfo.filterable =
				...
				bFilterable &&
				this._isFilterableNavigationProperty(oColumnInfo, oMetaModel) &&
